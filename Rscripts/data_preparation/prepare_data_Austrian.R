...
setwd('/mnt/f/Brinkman group/current/Stepan/Austrian')
...
for (i in 1:length(files_to_process)) {
  ...
  # cluster labels
  data_i <- cbind(data_i, label = rep( gsub("\\..*","", sub(".*/", "",files_to_process[i])), nrow(data_i)))
  print(dim(data_i))
...
table(data[,'label'])[order(table(data[,'label']))]
#########################
### ARCSINH TRANSFORM ###
#########################
...
cols_to_scale <- 1:38
#data[, cols_to_scale] <- asinh(as.numeric(data[, cols_to_scale]) / asinh_scale)

data[, cols_to_scale] <-as.numeric(data.matrix(data[, cols_to_scale]))
data[, cols_to_scale] <- apply((data[, cols_to_scale]), 2, function(x) asinh(x)/ asinh_scale)

summary(data)
#check for duplicates
zzz=data[!duplicated(data[,cols_to_scale]),]
dim(zzz)
...
data[,1:38]<-apply(data[,1:38], 2, as.numeric)
zzz<-data[duplicated(data[,1:38]),]
table(zzz[,"label"])
temp<-data[data$label %in% c('CD4_CM', 'CD4_EM', 'CD4_EMRA', 'CD4_Naive', 'CD8_CM', 'CD8_EM', 'CD8_EMRA', 'CD8_Naive' ), ]
temp2<-data[data$label %in% c('CD4' , 'CD8'), ]
table(temp2$label)
CD<-(rbind(temp,temp2))
CD<-CD[!duplicated(CD[,1:38]),]
table(temp$label)
table(CD$label)
dim(CD)
temp3 = data[data$label=='T', ]
Tc = (rbind(CD,temp3))
Tc<-Tc[!duplicated(Tc[,1:38]),]
table(temp3$label)
table(Tc$label)
dim(Tc)
temp4 = rbind(Tc, data[data$label %in% c('NK', 'NKT', 'NonT'), ])
HM = rbind(temp4,data[data$label=='HM', ])
HM<-HM[!duplicated(HM[,1:38]),]
write.table(HM$label, file = paste0("./WangDataPatient/","rln4NUmlbls",".txt"), quote = FALSE, sep = "\t", row.names = FALSE)
dim(HM)
#From preprocessWang
#value_cut<-unlist(lapply(colnames(dbexprs2), function(x) median(dbexprs2[bc_ids!=x & bc_ids!=0 & (dbexprs2[ ,x]!=0) ,x]) ))
#remove all signal below level of median bids noise
HM2 <- t(apply(HM[,1:38], 1, function(x) ifelse(x-value_cut>0, x, 0)))
HM2[, cols_to_scale] <- apply((HM2[, cols_to_scale]), 2, function(x) asinh(x)/ asinh_scale)

#normalize to interval [0..1]
HMn <- apply(HM[,1:38], 2, function(x) (x-min(x))/(max(x)-min(x)))

table(HM$label)
library(Rtsne)
Y=Rtsne(as.matrix(HM[,1:38]), check_duplicates=FALSE, n_jobs=12)
library(ggplot2)
df=as.data.frame(Y$Y)
ggplot(df, aes(x=df[,1], y=df[,2], color=HM[,39])) +
  geom_point()

Y2=Rtsne(as.matrix(temp4[,1:38]), check_duplicates=FALSE, n_jobs=12)
Cdf=as.data.frame(Y2$Y)
ggplot(Cdf, aes(x=Cdf[,1], y=Cdf[,2], color=temp4[,39])) +
  geom_point()
...
dim(data)
write.table(HM, file = paste0("./WangDataPatient/","rln4Asin5Transform",".txt"), quote = FALSE, sep = "\t", row.names = FALSE)
write.table(HM2, file = paste0("./WangDataPatient/","rln4Asin5TransformDenoise",".txt"), quote = FALSE, sep = "\t", row.names = FALSE)
#HM=read.table(file = paste0("./WangDataPatient/","rln4Asin5Transform",".txt"), header = T)
#HM=read.table(file = paste0("./WangDataPatient/","rln4NoTransform",".txt"), stringsAsFactors = F)
labelling = data.frame(num=table(as.integer(as.factor(HM[,39]))), name=table((HM[,39])))
labels = data.frame(num=(as.integer(as.factor(HM[,39]))))
write.table(labelling, file = paste0("./WangDataPatient/","rln4NUmlbls",".txt"), quote = FALSE, sep = "\t", row.names = FALSE)
