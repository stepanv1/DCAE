...
#########################################################################################
#ATTENTION corrected for duplicates, Stepan Grinek 
...
files <- list.files("/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/CyTOF AML PhenoGraph Benchmark Data Set 2 (see Fig S2D and Data S1G-I)/attachments/raw_data", pattern = "\\.fcs$", full.names = TRUE)
...
data_combined<-data_combined[!duplicated(data_combined[,5:36]), ]
dim(data_combined)

data_combined_notransform<-data_combined_notransform[!duplicated(data_combined_notransform[,5:36]), ]
dim(data_combined_notransform)
# export cell population names

write.table(df_pop_names, file = "/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/population_names_Levine_32dim.txt", quote = FALSE, sep = "\t", row.names = FALSE)

# save data files in TXT format

write.table(data_combined, file = "/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/Levine_32dim.txt", quote = FALSE, sep = "\t", row.names = FALSE)
write.table(data_combined_notransform, file = "/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/Levine_32dim_notransform.txt", quote = FALSE, sep = "\t", row.names = FALSE)

# save data files in FCS format

flowCore::write.FCS(flowCore::flowFrame(data_combined), filename = "/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/Levine_32dim.fcs")
flowCore::write.FCS(flowCore::flowFrame(data_combined_notransform), filename = "/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/Levine_32dim_notransform.fcs")
