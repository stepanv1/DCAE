...
is_subsampled <- c(FALSE, FALSE, FALSE, FALSE,  FALSE, FALSE)

# alternate FlowCAP results at the end
is_rare    <- c(FALSE, FALSE, FALSE, FALSE,  FALSE, FALSE)
is_FlowCAP <- c(FALSE, FALSE, FALSE, FALSE,  TRUE,  TRUE)
n_FlowCAP <- 2
...
i=2
png(filename = '../../plots/FLOCK/FLOCK_Levine_13.png', width = 1440, height = 1000)
ncolors=length(unique(cl_tr[[i]]))
col_true=colorRampPalette(c("violet","blue",  "yellow", "orange", "green","red"))(ncolors)
names(col_true) = unique(cl_tr[[i]])
...
xres<-xtable(df,  caption = "FLOCK results", digits=c(0,0,4,4,4,2,2), display=c('s', 's','fg','fg','fg','d','d'), include.rownames=FALSE, align = c("c", "c ",  "c ", "c ", "c ", "c ", "c "), label = "tab:FLOCK")
