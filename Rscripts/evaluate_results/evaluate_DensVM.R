...
cl_tr<-lapply(1:4, function(x) clus_truth[[x]][!(is.na(clus_truth[[x]]) | is.nan(clus_truth[[x]]))])

tsne3D<-lapply(1:4, function(x) res_tsne[[x]]$tsne_out3D$Y[!(is.na(clus_truth[[x]]) | is.nan(clus_truth[[x]])),])
tsne2D<-lapply(1:4, function(x) res_tsne[[x]]$tsne_out$Y[!(is.na(clus_truth[[x]]) | is.nan(clus_truth[[x]])),])
...
i=2
png(filename = '../../plots/DenseVM/prot01_Levine_13.png', width = 1440, height = 1000)
ncolors=length(unique(cl_tr[[i]]))
col_true=colorRampPalette(c("violet","blue",  "yellow", "orange", "green","red"))(ncolors)
names(col_true) = unique(cl_tr[[i]])
...
xres<-xtable(df,  caption = "DensVM 0.1 results", digits=c(0,0,4,4,4,2,2), display=c('s', 's','fg','fg','fg','d','d'), include.rownames=FALSE, align = c("c", "c ",  "c ", "c ", "c ", "c ", "c "), label = "tab:DensVM")
