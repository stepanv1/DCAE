...
files_truth <- list(
    ...
    Samusik_all = file.path(MANUAL_PHENOGRAPH, CALC_NAME,"Python_true_labels_phenoGraph_Samusik_all.txt")
...
library(NMI)
#NMIscore<-unlist(lapply(1:length(clus), function(i) NMI(cbind(1:length(clus[[1]]), clus[[i]]), cbind(1:length(clus[[1]]),unlist(clus_truth)))))
NMIscore<-c(0.890575868713,
0.894333818038,
0.89482156605,
0.894260403544,
0.89418488016,
...
#system.time(res_tsne<-mclapply(1, function(x){
    idx=sample(1:length((clus_truth[[1]])), 100000)
    ncolors=length(unique(clus_truth[[1]]))
    col=rainbow(ncolors)
    colors<-(unlist(lapply(clus_truth[[1]], function(x) col[x])))#[idx]
    tsne_out <- Rtsne(data[,  1:(ncol(data)-1)])
    tsne_out3D <- Rtsne(data[,  1:(ncol(data)-1)], dims=3)
