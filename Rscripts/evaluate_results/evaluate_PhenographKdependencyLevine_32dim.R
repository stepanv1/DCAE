...
CALC_NAME="KdependencyLevine_32dim"




#load  data
data=read.table(paste0(MANUAL_PHENOGRAPH , '/',CALC_NAME,"/data_Levine_32dim.txt"), header = F, stringsAsFactors = FALSE)
...
files_truth <- list(
  ...
  Levine_32dim = file.path(MANUAL_PHENOGRAPH, CALC_NAME,"Python_true_labels_phenoGraph_Levine_32dim.txt")
...
# load cluster labels
files_res <- list(
  Levine_32dim15 = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=15Python_assigned_labels_phenoGraph_Levine_32dim.txt"), 
  Levine_32dim30 = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=30Python_assigned_labels_phenoGraph_Levine_32dim.txt"), 
  Levine_32dim45   = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=45Python_assigned_labels_phenoGraph_Levine_32dim.txt"), 
  Levine_32dim60  = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=60Python_assigned_labels_phenoGraph_Levine_32dim.txt"),
  Levine_32dim75  = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=75Python_assigned_labels_phenoGraph_Levine_32dim.txt"),
  Levine_32dim90  = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=90Python_assigned_labels_phenoGraph_Levine_32dim.txt"),
  Levine_32dim105  = file.path(RES_DIR_PHENOGRAPH, CALC_NAME, "k=105Python_assigned_labels_phenoGraph_Levine_32dim.txt")
)
...
pop_names=read.table("../../benchmark_data_sets/attachments/population_names_Levine_32dim.txt", header = T, stringsAsFactors = FALSE, sep='\t')
...
#find markers differentiated in population(s) in B-cells
...
plot3d(sset[,c("HLA-DR", "CXCR4",  "CD44")], pch='.', col=colors[IDXsset])
hist(sset[, 'CD20'], 50)
hist(data[, 'CD20'], 500)
...
cells=c(12,13,14)
IDXsset<-clus_truth %in% cells
clus_vec<-ifelse(IDXsset, clus_truth[IDXsset], -runif(1))#need subset the 'bulk'
sub_clus_vec<-ifelse(clus_vec>-0.1, T, F)
sset<-data[, 1:32]
#plot3d(sset[,c('IgD', 'IgM', 'MHCII')])
ndc<-ncoord(sset, sub_clus_vec, weighted=F)
plot3d(ndc$proj[,1:3], col=colors[IDXsset])
units<-ndc$units[,1] 
colnames(sset)[order(abs(units),decreasing=T)][1:9]
top_comp<-colnames(sset)[order(abs(units),decreasing=T)][c(1,2,3)]
