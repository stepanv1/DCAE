...
#create 2D and 3D plots with legends
####################################
#load true labels
...
colors1<-unlist(lapply(clus_truth[[1]], function(x) ifelse(x==19, col_true[as.character(x)], 'black')))
#data<- cbind(f, clusters)
plot(res_tsne[[1]]$tsne_out$Y,col=colors1, pch='.', cex=1)
plot3d(res_tsne[[1]]$tsne_out3D$Y,  col=colors1, pch='.') 
