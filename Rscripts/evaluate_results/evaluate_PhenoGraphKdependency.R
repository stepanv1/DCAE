...
i=7
ncolors=length(unique(clus_truth[[1]]))
col_true=colorRampPalette(c("violet","blue",  "yellow", "orange", "green","red"))(ncolors)
names(col_true) = unique(clus_truth[[1]])
colors<-unlist(lapply(clus_truth[[1]], function(x) col_true[as.character(x)]))
#data<- cbind(f, clusters)
par(oma = c(1, 1, 1, 6))
#par(fig = c(0, 1, 0, 1), oma = c(0, 0, 0, 0), mar = c(0, 0, 0, 0), new = TRUE)
plot(res_tsne[[1]]$tsne_out$Y,col=colors, pch='.', cex=1, xlab='tsne 1', ylab='tsne 2')
legend(x = 'right', bty='n', inset=c(-0.2,0), pop_names$population, col = col_true[as.character(pop_names$label)], pch = 16, xpd=TRUE, cex=0.8, pt.cex=1.5)

plot3d(res_tsne[[1]]$tsne_out3D$Y,  col=colors, pch='.', xlab='tsne 1', ylab='tsne 2', zlab='tsne 3') 
legend3d("topright", legend = pop_names$population, pch = 16, col = col_true[as.character(pop_names$label)], cex=2, inset=c(0.02))

#color only one cluster  25 13 18 11 12  1  5 17 19 24 21  7  2 23  8
cells=25
colors1<-unlist(lapply(clus_truth[[1]], function(x) ifelse(x==cells, col_true[as.character(x)], 'black')))
#data<- cbind(f, clusters)
plot(res_tsne[[1]]$tsne_out$Y,col=colors1, pch='.', cex=1, main = pop_names[pop_names$label==cells,'population'], xlab='tsne 1', ylab='tsne 2')
plot3d(res_tsne[[1]]$tsne_out3D$Y,  col=colors1, pch='.', main = pop_names[pop_names$label==cells,'population'], xlab='tsne 1', ylab='tsne 2', zlab='tsne 3') 
...
plot(res_tsne[[1]]$tsne_out$Y,col=colors_matched, pch='.', cex=1, , xlab='tsne 1', ylab='tsne 2')
...
plot3d(res_tsne[[1]]$tsne_out3D$Y,  col=colors_matched, pch='.', xlab='tsne 1', ylab='tsne 2', zlab='tsne 3')
...
plot(res_tsne[[1]]$tsne_out$Y,col=colors_auto, pch='.', cex=1, xlab='tsne 1', ylab='tsne 2')
legend(x = 'right', bty='n', inset=c(-0.28,0), legend=unique(clus[[i]]), col = col_auto, pch = 16, xpd=TRUE, cex=0.8, pt.cex=1.5)
plot3d(res_tsne[[1]]$tsne_out3D$Y,  col=colors_auto, pch='.', xlab='tsne 1', ylab='tsne 2', zlab='tsne 3') 
legend3d("topright",  inset=c(0.02), legend=unique(clus[[i]]), col = col_auto, pch = 16, xpd=TRUE,  cex=1)
