...
files_truth <- list(
  ...
  Samusik_all  = file.path(RES_DIR_XSHIFT, "Samusik_all_notransform_subsampled300k.fcs")#, 
...
files_out <- list(
  Levine_32dim = file.path(RES_DIR_XSHIFT, "Levine32.csv"), 
  Levine_13dim = file.path(RES_DIR_XSHIFT, "Levine13.csv"), 
  Samusik_01   = file.path(RES_DIR_XSHIFT, "Samusik_01.csv"), 
  Samusik_all  = file.path(RES_DIR_XSHIFT, "Samusik_all300k.csv")#, 
...
for (i in 2) {
  data_i <- read.csv(files_out[[i]], stringsAsFactors = FALSE, header = TRUE)
