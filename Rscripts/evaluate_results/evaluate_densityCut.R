...
RES_DIR_DENSITYCUT <- "../../results/auto/densityCut"
...
# load cluster labels
files_res <- list(
    Levine_32dim = file.path(paste0(RES_DIR_DENSITYCUT, "/", CALC_NAME), "Levine_32dim.txt"), 
    Levine_13dim = file.path(paste0(RES_DIR_DENSITYCUT, "/", CALC_NAME), "Levine_13dim.txt"), 
    Samusik_01   = file.path(paste0(RES_DIR_DENSITYCUT, "/", CALC_NAME), "Samusik_01.txt"), 
    Samusik_all  = file.path(paste0(RES_DIR_DENSITYCUT, "/", CALC_NAME), "Samusik_all.txt"), 
    Nilsson_rare = file.path(paste0(RES_DIR_DENSITYCUT, "/", CALC_NAME), "Nilsson_rare.txt"), 
    Mosmann_rare = file.path(paste0(RES_DIR_DENSITYCUT, "/", CALC_NAME), "Mosmann_rare.txt") 
    #FlowCAP_ND   = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_FlowCAP_ND.fcs"), 
    #FlowCAP_WNV  = file.path(RES_DIR_DENSITYCUT, "densityCut_labels_FlowCAP_WNV.fcs")
)
...
i=2
png(filename = '../../plots/densityCut/densityCut_Levine_13.png', width = 1440, height = 1000)
ncolors=length(unique(cl_tr[[i]]))
col_true=colorRampPalette(c("violet","blue",  "yellow", "orange", "green","red"))(ncolors)
names(col_true) = unique(cl_tr[[i]])
