...
#load  data
data=read.table(paste0(MANUAL_PHENOGRAPH , '/','KdependencySamusik_NAN',"/data_Samusik_all.txt"), header = F, stringsAsFactors = FALSE)
...
files_truth <- list(
  Samusik_01 = file.path(DATA_DIR, "Samusik_all.fcs") 
)
...
unassign<-is.na(clus_truth[[1]])
#clus_truth[[1]] <- clus_truth[[1]][!unassign]
...
# load cluster labels
files_res <- list(
  Samusik_all = file.path(RES_DIR, CALC_NAME, "clusForestBalance.txt") 
...
resAMI <- lapply(1:7, function(i) AMI(clus[[i]], clus_truth[[1]], mc.cores=4))
resAMI
...
clus_truth<-clus_truth[[1]][!unassign]
clus<-clus[[1]][!unassign]
...
match_table <-res[[1]]$labels_matched
lbl_mapped = unlist(lapply(clus, function(x) {
...
ncolors_auto=length(unique(clus))
col_auto=sample(rainbow(ncolors_auto), ncolors_auto , replace=F)
names(col_auto) = unique(clus)
colors_auto<-unlist(lapply(clus, function(x) col_auto[as.character(x)]))
plot(tsne2D,col=colors_auto, pch='.', cex=1)
plot3d(tsne3D,  col=colors_auto, pch='.') 

######################################################################
#comparison with phenograph

clus2 <- read.table('/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/results/auto/phenoGraph/KdependencySamusik_NAN/k=45Python_assigned_labels_phenoGraph_Samusik_all.txt', header = F, stringsAsFactors = FALSE)[,1]
clus2<-clus2[!unassign]
res[[2]]<-helper_match_evaluate_multiple(clus2, clus_truth)
#compare other measures
resN1<-helper_match_evaluate_multiple_SweightedN(clus, clus_truth)
resN2<-helper_match_evaluate_multiple_SweightedN(clus2, clus_truth)

AMI(as.integer(clus), clus_truth, mc.cores=2)
AMI(as.integer(clus2), clus_truth, mc.cores=2)
...
#match the clusters and plot F_1 consensus results 
match_table <-res[[2]]$labels_matched
lbl_mapped = unlist(lapply(clus2, function(x) {
  ifelse(length(which(x==match_table))==0, 0, as.numeric(names(match_table[which(x==match_table)])))}))
colors_matched<-unlist(lapply(lbl_mapped, function(x) ifelse(x==0, 'black', col_true[as.character(x)])))
plot(tsne2D,col=colors_matched, pch='.', cex=1)
legend(x = 'right', bty='n', inset=c(-0.28,0), c(pop_names$population, 'Unassigned'), col = c(col_true[as.character(pop_names$label)], 'black'), pch = 16, xpd=TRUE, cex=0.8, pt.cex=1.5)
...
##############################################################################################
#unsupervised measures of clusterin
library(factoextra)
library(cluster)
library(fpc)
library(NbClust)
clus<-unlist(read.table(paste0(RES_DIR, '/KdependencySamusik_840k/', 'clusForestBalancever2.txt'), header = T, stringsAsFactors = FALSE))
library("cluster")
IDX=sample(1:length(as.integer(unlist(clus))),100000)
sil <- silhouette(as.integer(clus[IDX]), dist(data[[4]][IDX,],  method = "euclidean"))
...
df<-matrix(unlist(lapply(1:length(clus), function(x) {z=res[[x]]; c(as.integer(rnames[x]), as.numeric(z$mean_F1), as.numeric(NMIscore[x]), as.numeric(z$mean_re), as.numeric(z$mean_pr),  as.integer(z$n_clus), as.integer(length(unique(clus_truth[[1]]))-1))})), ncol=7, byrow=T )
