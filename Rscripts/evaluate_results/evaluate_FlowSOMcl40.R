...
MANUAL_FLOWSOM <- "../../results/manual/FlowSOM"
RES_DIR_FLOWSOM <- "../../results/auto/FlowSOM"
...
#load all data sets
load(file=paste0(DATA_DIR, '/all_sets.RData'))
...
files_out <- list(
    Levine_32dim = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_Levine_32dim.txt"), 
    Levine_13dim = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_Levine_13dim.txt"), 
    Samusik_01   = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_Samusik_01.txt"), 
    Samusik_all  = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_Samusik_all.txt"), 
    Nilsson_rare = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_Nilsson_rare.txt"), 
    Mosmann_rare = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_Mosmann_rare.txt") 
    #FlowCAP_ND   = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_FlowCAP_ND.txt"), 
    #FlowCAP_WNV  = file.path(MANUAL_FLOWSOM, "FlowSOM_labels_FlowCAP_WNV.txt")
)
...
dirname=paste0("../../plots/FlowSOM/")
dir.create(file.path(dirname), showWarnings = T)
par(mar=c(5,4,4,2)+0.1)
for (k in 1:4){
    png(filename=paste0(dirname, '/', names(data[k]),  'Mosaic40cl.png' ))
    tab=(table(clus_truth[[k]], clus[[k]],dnn=c("True","Assign")))
    rownames(tab)        = sort(unique(clus_truth[[k]]))
...
par(mar=c(5,4,4,2)+0.1)
for (k in 1:4){
    png(filename=paste0(dirname, '/', names(data[k]),  'Mosaic40clRotate.png' ))
    tab=(table(clus[[k]], clus_truth[[k]],dnn=c("Assign", "True")))
    rownames(tab)        = sort(unique(clus[[k]]))
    colnames(tab)        = sort(unique(clus_truth[[k]]))
    #tab=tab[,colSums(tab != 0) != 0] 
    mosaic(tab, shade=F, legend=F, cex=0.5, cex.lab=0.2,  pop = FALSE, zero_size=0.5, direction='h', labeling_args=list(rot_labels=c(bottom=90,top=90),gp_labels=(gpar(fontsize=6))))
    dev.off()    
}
...
png(filename = '../../plots/FlowSOM/FlowSOM_Levine_13.png', width = 1440, height = 1000)
ncolors=length(unique(cl_tr[[i]]))
col_true=colorRampPalette(c("violet","blue",  "yellow", "orange", "green","red"))(ncolors)
names(col_true) = unique(cl_tr[[i]])
...
xres<-xtable(df,  caption = "FlowSOM results", digits=c(0,0,4,4,4,2,2), display=c('s', 's','fg','fg','fg','d','d'), include.rownames=FALSE, align = c("c", "c ",  "c ", "c ", "c ", "c ", "c "), label = "tab:flowMeans")
