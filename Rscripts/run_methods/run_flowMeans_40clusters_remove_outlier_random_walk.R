...
# using random walk. Flowmeans at fixed number of clusters. Siluette index.
...
for (i in 1:length(files)){
  ...
  #################################################################################
  system.time(resflow<-flowMeans(data[[i]],  NumC = 40, Standardize = FALSE)@Label)
  table(resflow)
  ...
  #run on modified set
  system.time(cl_res_walk<-flowMeans(data[[i]][outliersWalk[[i]], ], NumC = 40, Standardize = FALSE)@Label) ;gc()
  table(cl_res_walk)
  ...
  system.time(cl_resi<-flowMeans(data[[i]][IDX , ], NumC = 40, Standardize = FALSE)@Label) ;gc()
...
save(outliers2Step, outliersWalk, clus_assign, clus_assign_2Step, clus_assign_walk, file=paste0(RES_DIR, '/', CALC_NAME, '.RData'))

load( file=paste0(RES_DIR, '/', CALC_NAME, '.RData'))

###############################################################################################
#assignment of outliers to clusters and evaluation
#using silhouette and F1 measure. flowMeans algorithm is run
...
#sample non-outlers
ii<-sample(outliersWalk[[i]], 10000)
...
sind1<-silhouette(clus_assign[[i]][idxs], dist(scale(data[[i]][idxs,])))
plot(sind1)
median(sind1[ ,3])

sind2<-silhouette(RFassignWalk[[i]][ii], dist(scale(data[[i]][ii,])))
plot(sind2)
median(sind2[ ,3])
