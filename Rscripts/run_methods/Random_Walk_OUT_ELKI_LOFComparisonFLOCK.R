...
for (i in 1:length(outliersWalkP)){
  system.time(resflow<-helper_call_Flock(data[[i]][outliersWalkP[[i]]!=0,], FLOCKDIR = '/home/sgrinek/bin/FLOCK')$Population)
  table(resflow)
...
load(file=paste0(RES_DIR, '/', CALC_NAME, '_', 'clusterOutImprove', '.RData'))
...
for (i in 1:length(outliersWalkP)){
  ...
  system.time(resflow<-helper_call_Flock(data[[i]][ixOUTRANK,], FLOCKDIR = '/home/sgrinek/bin/FLOCK')$Population)
  gc()
  ...
  system.time(resflow<-helper_call_Flock(data[[i]][ixSOD,], FLOCKDIR = '/home/sgrinek/bin/FLOCK')$Population)
  gc()
...
xres<-xtable(PerfMeasuresAllsets,  caption = "Tests results with FLOCK, 4 methods", digits=4, display=c('s', 's', 'fg','fg','fg','fg', 'fg'), include.rownames=FALSE, align = c("|c|c|c|c|c|c|c|"), label = "FLOCK_4methods")
