...
#find subdimensions and outlierness measures
for(i in 1:length(files_truth)){
  ref_subs[[i]]<-helper_find_subdimensions(data[[i]], clus_assign[[i]])$subdim
}
lapply(data,dim)
lapply(clus_assign, length)

X11();heatmap.2(as.matrix(ifelse(ref_subs[[1]],1,0)))
X11();beanplot(as.data.frame(d[clr==32, ref_subs[[1]][32,]]))
table(clus_assign[[1]])
##########################################################################
# find density and localisation measures
#for(i in 1:length(files_truth)){
############################################################################
for(i in 1:2){
  d<-data[[i]]
  rs<-ref_subs[[i]]
  cl<-clus_assign[[i]]
  system.time(GO<-helper_global_outliers_Uniform(d, rs, cl, nbins=10, mc.cores=10))
  print(system.time(LO<-mclapply(1:nrow(rs), function(x) helper_local_outliersLOF2(d[, rs[x,]],  cl==x, k=30), mc.cores=10, mc.preschedule=F)))
  #combo, LO*G0
  GOm = Reduce(cbind, GO) 
  LOm = Reduce(cbind, (lapply(LO, function(x) x$prob))) 
  CO=as.matrix(GOm*LOm)
  colnames(CO)=1:nrow(rs)
...
#save( GOmat, LOmat, COmat, localisation, max_density,  file=paste0(RES_DIR,'/Conf_out_data1.RData'))
#load(file=paste0(RES_DIR,'/Conf_out_data1.RData'))
gc()


############################################################################
#calculation for inter and intra cluster ouliers 
for(i in 1:2){
  GOm<- GOmat[[i]]; LOm<- LOmat[[i]]; CO<-COmat[[i]]; LD<-localisation[[i]]
  MD <- max_density[[i]]
  d<-data[[i]]
  rs<-ref_subs[[i]]
  cl<-clus_assign[[i]]
  ...
  cut_offGO<- unlist(mclapply(1:ncol(GOm), function(x) helper_topPercentile(0.05,GOm[cl==x,x])$lim_cut))
  cut_offLO<- unlist(mclapply(1:ncol(LOm), function(x) helper_topPercentile(0.05,LOm[cl==x,x])$lim_cut))
  #inGO <- unlist(mclapply(1:nrow(GOm), function(x) {
  #  all(GOm[x,]<cut_offGO & LOm[x,]<cut_offLO);
  #}, mc.cores=10)  )
  inGO <- unlist(mclapply(1:nrow(GOm), function(x) {
    LOm[x,cl[x]]<0.05;
  }, mc.cores=10))
  sum(inGO)
  table(clus_truth[[i]][inGO])
  sum(table(clus_truth[[i]][inGO]))
  table(cl[inGO])
  inter<- MD<0.05 & LD < 0.6
  sum(inter)
  table(clus_truth[[i]][inter])
  sum(table(clus_truth[[i]][inter]))
  table(cl[inter])
  
  clust_clean <- !(inter | inGO) 
  
  sum(!clust_clean)
  table(clus_truth[[i]][clust_clean])
  table(clus_truth[[i]])
  sum(table(clus_truth[[i]]))
  sum( table(clus_truth[[i]][clust_clean]))
  #############
  #clustering
  ...
  #} else {
  print(system.time(g<-create_snnk_graph(d[clust_clean,], k,  metric='L2')$graph))
  
  
  print(system.time(cl2<- louvain_multiple_runs_par(g, num.run=5, mc.cores=5)))
  
  graph[[i]] <-  g 
  cluster_assign[[i]] <- cl2 
  clust_out[[i]] <- clust_clean
  
  print(rescl<-helper_match_evaluate_multiple(cl, clus_truth[[i]]))
  print(rescl2<-helper_match_evaluate_multiple(cl2, clus_truth[[i]][clust_clean]))
  print(AMI(cl, clus_truth[[i]]))
  print(AMI(cl2, clus_truth[[i]][clust_clean]))
  helper_match_evaluate_multiple_SweightedN(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_SweightedN(cl2, clus_truth[[i]][clust_clean])
  helper_match_evaluate_multiple_MoCu(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_MoCu(cl2, clus_truth[[i]][clust_clean])
  X11();plot((clus_truth[[i]]+rnorm(length(clus_truth[[i]]), sd=0.2))[clust_clean], cl2 +rnorm(length(cl2), sd=0.2), pch = '.', col = alpha('red', 0.5))
  X11();plot((clus_truth[[i]]+rnorm(length(clus_truth[[i]]), sd=0.2)), cl +rnorm(length(cl), sd=0.2), pch = '.', col = alpha('red', 0.5))
  
  invlblm<-as.numeric(names(rescl2$labels_matched))
  names(invlblm)<-rescl2$labels_matched
  ncl2<-rescl$labels_matched[as.character(invlblm[as.character(cl2)])]
  table(ncl2)
  oldidx<-(1:rescl2$n_clus)[!(1:rescl2$n_clus %in% as.numeric(names(table(ncl2))))]
  leftmatch<-oldidx
  names(leftmatch)<-names(table(cl2[is.na(ncl2)]))[1:length(table(cl2[is.na(ncl2)]))]
  leftmatch
  ncl2[is.na(ncl2)]<- as.integer(leftmatch[as.character(cl2[is.na(ncl2)])])
  
  X11();plot((clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=0.2)))[clust_clean], ncl2 +abs(rnorm(length(ncl2), sd=0.2)), pch = '.', col = alpha('red', 0.8), ylim=c(0,length(table(cl))))
  points((clus_truth[[i]]-(rnorm(length(clus_truth[[i]]), sd=0.2))), cl -abs(rnorm(length(cl), sd=0.2)), pch = '.', col = alpha('black', 0.8))
  ...
  leftmatch
  ncl2[is.na(ncl2)]<- as.integer(leftmatch[as.character(cl2[is.na(ncl2)])])
  #reassign for a graph
  ncl2r <- ncl2; ncl2r[ncl2r==32]<-25; ncl2r[ncl2r==30]<-27;ncl2r[ncl2r==29]<-28;
  clr <-cl; idx32<- clr==32; idx25<- clr==25;idx27<- clr==27;
  idx28<- clr==28; idx29<- clr==29;idx30<- clr==30; clr[idx32]<-25;
  clr[idx30]<-27;clr[idx29]<-28;  clr[idx25]<-32; clr[idx27]<-30;clr[idx28]<-29;
  
  hl<-!is.na(clus_truth[[i]])
  X11();plot((clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=0.2)))[clust_clean], ncl2 +abs(rnorm(length(ncl2), sd=0.2)), pch = '.', col = alpha('red', 1), ylim=c(0,length(table(cl))))
  points((clus_truth[[i]]-(rnorm(length(clus_truth[[i]]), sd=0.2))), cl -abs(rnorm(length(cl), sd=0.2)), pch = '.', col = alpha('black', 1))
  
  pltC<-data.frame(x=(clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=0.2)))[clust_clean],y=ncl2r +abs(rnorm(length(ncl2r), sd=0.2)));pltC <- na.omit(pltC)
  X11();plot(pltC, pch = '.', col = alpha('red', 1), ylim=c(0,length(table(cl))))
  plt<-data.frame(x=(clus_truth[[i]]-(rnorm(length(clus_truth[[i]]), sd=0.2))),y=clr -abs(rnorm(length(clr), sd=0.2)));plt <- na.omit(plt)
  points(plt, pch = '.', col = alpha('black', 1))
  ey<-clr[clr %in% c(29:32)]+rnorm(length(clr[clr %in% c(29:32)]), sd=0.2)
  ex<-3*((clr[clr %in% c(29:32)])-28) + rnorm(length(clr[clr %in% c(29:32)]), sd=0.2)
  points(x=ex, y=ey, pch = '.', col = alpha('black', 1))
  abline(a=28, b=0)
  
 plot3d(d[clr %in% c(32,9,22) ,c('CD7', 'CD4', 'CD3')], col =clr[clr %in% c(32,9,22)] )
 plot3d(d[clr %in% c(9,22) ,c('CD7', 'CD4', 'CD3')], col =clr[clr %in% c(9,22)] )
  
}
load(file=paste0(RES_DIR, 'cluster_assign1.RData'))
i=1
cl<-clus_assign[[i]];cl2<-cluster_assign[[i]];clust_clean<-clust_out[[i]]

#save(clust_out, graph, cluster_assign, file=paste0(RES_DIR, 'cluster_assign1.RData'))


############################################################################
#calculation for inter 
for(i in 1:2){
  GOm<- GOmat[[i]]; LOm<- LOmat[[i]]; CO<-COmat[[i]]; LD<-localisation[[i]]
  MD <- max_density[[i]]
  d<-data[[i]]
  ...
  #identifying outliers
  cut_offGO<- unlist(mclapply(1:ncol(GOm), function(x) helper_topPercentile(0.99,GOm[cl==x,x])$lim_cut))
  cut_offLO<- unlist(mclapply(1:ncol(LOm), function(x) helper_topPercentile(0.02,LOm[cl==x,x])$lim_cut))
  #inGO <- unlist(mclapply(1:nrow(GOm), function(x) {
  #  all(GOm[x,]<cut_offGO & LOm[x,]<cut_offLO);
  #}, mc.cores=10)  )
  inGO <- unlist(mclapply(1:nrow(GOm), function(x) {
  ...
  graph[[i]] <-  g 
  cluster_assign2[[i]] <- cl2 
  clust_out2[[i]] <- clust_clean
  ...
  X11();plot((clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=0.2)))[clust_clean], ncl2 +abs(rnorm(length(ncl2), sd=0.2)), pch = '.', col = alpha('red', 0.5), ylim=c(0,length(table(cl))))
  points((clus_truth[[i]]-(rnorm(length(clus_truth[[i]]), sd=0.2))), cl -abs(rnorm(length(cl), sd=0.2)), pch = '.', col = alpha('black', 0.5))
  
}
load(file=paste0(RES_DIR, 'cluster_assign2.RData'))
i=1
cl<-clus_assign[[i]];cl2<-cluster_assign2[[i]];clust_clean<-clust_out2[[i]]

#save(clust_out2, graph2, cluster_assign2, file=paste0(RES_DIR, 'cluster_assign2.RData'))


############################################################################
#calculation for intra cluster ouliers 
for(i in 1:2){
  GOm<- GOmat[[i]]; LOm<- LOmat[[i]]; CO<-COmat[[i]]; LD<-localisation[[i]]
  MD <- max_density[[i]]
  d<-data[[i]]
  ...
  graph3[[i]] <-  g 
  cluster_assign3[[i]] <- cl2 
  clust_out3[[i]] <- clust_clean
  ...
  pltC<-data.frame(x=(clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=0.2)))[clust_clean],y=ncl2r +abs(rnorm(length(ncl2r), sd=0.2)), col='red');pltC <- na.omit(pltC)
  X11();plot(pltC, pch = '.', col = alpha('red', 1), ylim=c(0,length(table(cl))))
  plt<-data.frame(x=(clus_truth[[i]]-(rnorm(length(clus_truth[[i]]), sd=0.2))),y=clr -abs(rnorm(length(clr), sd=0.2)), col='blue');plt <- na.omit(plt)
  points(plt, pch = '.', col = alpha('blue', 1))
  pltOut<-data.frame( x= 3*((clr[clr %in% c(29:32)])-28) + rnorm(length(clr[clr %in% c(29:32)]), sd=0.2), y = clr[clr %in% c(29:32)]+rnorm(length(clr[clr %in% c(29:32)]), sd=0.2), col='green')
  points(pltOut, pch = '.', col = alpha('green', 1))
  #abline(a=29, b=0)
  
  #more reassignments for final beautification
  idx13<- clr==13;idx9<- clr==9;
  clr[idx13]<-9;clr[idx9]<-13;
  idx4<- clr==4;idx22<- clr==22;
  clr[idx4]<-22;clr[idx22]<-4;
  idx17<- clr==17;idx19<- clr==19;
  clr[idx19]<-17;clr[idx19]<-17;
  idx13<- clr==13;idx4<- clr==4;
  clr[idx13]<-4;clr[idx4]<-13;
  idx2<- clr==2;idx28<- clr==28;
  clr[idx28]<-2;clr[idx2]<-28;
  idx6<- clr==6;idx24<- clr==24;
  clr[idx24]<-6;clr[idx6]<-24;
  idx18<- clr==18;idx19<- clr==19;
  clr[idx18]<-19;clr[idx19]<-18;
  idx11<- clr==11;idx28<- clr==28;
  clr[idx28]<-11;clr[idx11]<-28;
  #idx18<- clr==18;idx22<- clr==22;
  #clr[idx18]<-22;clr[idx22]<-18;
  
  sdp=0.15
  pltC<-data.frame(x=(clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=sdp)))[clust_clean],y=ncl2r +abs(rnorm(length(ncl2r), sd=sdp)), col='Clean');pltC <- na.omit(pltC)
  plt<-data.frame(x=(clus_truth[[i]]-(rnorm(length(clus_truth[[i]]), sd=sdp))),y=clr -abs(rnorm(length(clr), sd=sdp)), col='No cleaning');plt <- na.omit(plt)
  pltOut<-data.frame( x= 3*((clr[clr %in% c(29:32)])-28) + rnorm(length(clr[clr %in% c(29:32)]), sd=sdp), y = clr[clr %in% c(29:32)]+rnorm(length(clr[clr %in% c(29:32)]), sd=sdp), col='Outliers')
  d <- rbind(rbind(pltC,plt),pltOut)
  p<-ggplot(d[, ], aes(x=x, y=y,  color=col)) 
  values <- c(1:14)
    for (i in values) {
    p <- p + geom_segment(x=i, y=1, xend=i, yend=28, size=0.1 , colour="white")
  }
  p<-p+geom_point(size=0.05,alpha = 0.2) +scale_y_continuous(breaks=1:32)+scale_x_continuous(breaks=1:14)+theme(panel.grid.minor.x = element_blank(), panel.grid.major.x = element_blank(), panel.grid.minor.y = element_blank());
  
  
  X11(); p
  
  #plot of two clusterings together
  pltall<-data.frame(x=ncl2r +abs(rnorm(length(ncl2r), sd=0.2)), y=(clr -abs(rnorm(length(clr), sd=0.2)))[clust_clean]);
  X11();plot(pltall, pch = '.', col = alpha('red', 1), ylim=c(0,length(table(cl))))
}
load(file=paste0(RES_DIR, 'cluster_assign3.RData'))
i=1
cl<-clus_assign[[i]];cl2<-cluster_assign3[[i]];clust_clean<-clust_out3[[i]]
#save(clust_out3, graph3, cluster_assign3, file=paste0(RES_DIR, 'cluster_assign3.RData'))
...
sym_w<-(asym_w+t(asym_w))/2nnind105
