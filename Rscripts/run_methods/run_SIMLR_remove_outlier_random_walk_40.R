...
for (i in 1:length(files)){
  ...
  print('starting clustering')
  print(paste0('set # ', i))
  system.time(cl_res_walk<-SIMLR_Large_Scale(X=t(data[[i]][outliersWalk[[i]], ]), c=40, k = 30, kk = dim(data[[i]])[2], if.impute = FALSE, normalize = F)$cluster) ;gc()
  table(cl_res_walk)
