...
load( file=paste0(DATA_DIR, '/artSam.RData'))
#Frame=aFrame[lbls %in% 1:10, ]
...
save(clus_assign, file=paste0(DATA_DIR, '/artSam32Clus_assign.RData'))
...
table(lblsT)
save(Assign,ScoreLst, file=paste0(DATA_DIR, '/FULLartSamScores61.RData'))
AMIlist<-(unlist(lapply(Assign[1:61], function(x) AMI(x, as.numeric(as.factor(lblsT))))))
plot((sumScores/max(sumScores))[2:61], ylim=c(0,1))
points(AMIlist[1:60],  col='red')
cor((sumScores/max(sumScores))[2:15],AMIlist[1:14], method='spearman')
plot((sumScores/max(sumScores))[2:61], AMIlist[1:60])
