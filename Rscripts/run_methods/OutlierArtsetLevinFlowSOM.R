...
print(system.time(clus_assign<-helper_call_FlowSOM(DFtoFF(as.data.frame(data)), marker_cols=1:dim(data)[2])))
...
COlist<-mclapply(1:length(clus_assignM), 
                 function(x)  unlist(lapply(1:num_clus, function(y) ifelse(CO[x,y]<0.001 & clus_assignM[x]!=y, 0, CO[x,y] ) )), mc.cores=40) 
...
print(system.time(lblsNew<-helper_call_FlowSOM(DFtoFF(as.data.frame(dataNew)), marker_cols=1:dim(data)[2])))
...
table(lblsT)  
save(Assign,ScoreLst, file=paste0(DATA_DIR, '/artLevinFlowSom.RData'))
