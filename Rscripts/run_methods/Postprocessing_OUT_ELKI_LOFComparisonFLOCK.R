...
seed<-set.seed(12345)
#CALC_NAME = 'DELETE'
CALC_NAME <- 'POSTPROCESSING_RW_vs_LOF'
ALG_NAME='FLOCK'
...
for (i in 1:length(outliersWalkP)){
  clus_assign[[i]]<-as.integer(clus_assign[[i]])
  ixRW<-(1:length(outliersWalkP[[i]]))[outliersWalkP[[i]]!=0]
  ixLOF<-(1:length(outliersWalkP[[i]]))[frankv(outl_resld[[i]]$lof[,2], ties.method = 'random', order=-1) > sum(outliersWalkP[[i]]==0)]
  
  PerfMeasures[[i]] = matrix(NA, 3,2)
  PerfMeasures[[i]][1,1] <- helper_match_evaluate_multiple(clus_assign[[i]], clus_truth[[i]])$mean_F1 
  PerfMeasures[[i]][2,1] <- helper_match_evaluate_multiple(clus_assign[[i]][ixRW], clus_truth[[i]][ixRW])$mean_F1
  PerfMeasures[[i]][3,1] <- helper_match_evaluate_multiple(clus_assign[[i]][ixLOF], clus_truth[[i]][ixLOF])$mean_F1
  
  PerfMeasures[[i]][1,2] <- AMI(clus_assign[[i]], clus_truth[[i]])
  PerfMeasures[[i]][2,2] <- AMI(clus_assign[[i]][ixRW], clus_truth[[i]][ixRW])
  PerfMeasures[[i]][3,2] <- AMI(clus_assign[[i]][ixLOF], clus_truth[[i]][ixLOF])
  
  #idxs <- sample(length(clus_assign[[i]]), 20000)
  #int<-intCriteria(scale(data[[i]][idxs,]), clus_assign[[i]][idxs],c("C_index","Gamma"))
  
  #ii<-sample(ixRW, 20000)
  #intRW<-intCriteria(scale(data[[i]][ii,]), clus_assign[[i]][ixRW][ii],c("C_index","Gamma"))
  
  #ii<-sample(ixLOF, 20000)
...
for (i in 1:length(outliersWalkP)){
  ...
  PerfMeasuresELKI[[i]] = matrix(NA, 2,2)
  
  PerfMeasuresELKI[[i]][1,1] <- helper_match_evaluate_multiple(clus_assign[[i]][ixSOD], clus_truth[[i]][ixSOD])$mean_F1
  PerfMeasuresELKI[[i]][2,1] <- helper_match_evaluate_multiple(clus_assign[[i]][ixOUTRANK], clus_truth[[i]][ixOUTRANK])$mean_F1
  
  PerfMeasuresELKI[[i]][1,2] <- AMI(clus_assign[[i]][ixSOD], clus_truth[[i]][ixSOD])
  PerfMeasuresELKI[[i]][2,2] <- AMI(clus_assign[[i]][ixOUTRANK], clus_truth[[i]][ixOUTRANK])
  
  
  #ii<-sample(ixSOD, 20000)
  #intRW<-intCriteria(scale(data[[i]][ii,]), clus_assign[[i]][ixSOD][ii],c("C_index","Gamma"))
  
  #ii<-sample(ixOUTRANK, 20000)
  #intRW<-intCriteria(scale(data[[i]][ii,]), clus_assign[[i]][ixOUTRANK][ii],c("C_index","Gamma"))
...
PerfMeasures <- rbind(PerfMeasures, PerfMeasuresELKI)
...
goodNames<-c('Levine32', 'Levine13',  'SamusikAll')
setNames <- unlist(lapply(goodNames, function(x) c(x, '', '', '', '')))
methodNames<-rep(c('No detection', 'Random Walk', 'LOF', 'SOD', 'OUTRANK'), 3)
PerfMeasuresAllsets <-cbind(setNames, cbind(methodNames, PerfMeasuresAllsets)) 
colnames(PerfMeasuresAllsets)<-c('Data set', 'Outlier detection algorithm', 'mean F1', 'AMI')

xres<-xtable(PerfMeasuresAllsets,  caption = "Postprocessing tests results with FLOCK, 4 methods", digits=4, display=c('s', 's', 'fg','fg','fg'), include.rownames=FALSE, align = c("|c|c|c|c|c|"), label = "FLOCK_4methods")
