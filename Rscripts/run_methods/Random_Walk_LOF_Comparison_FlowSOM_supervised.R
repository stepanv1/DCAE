...
for (i in 1:length(outliersWalkP)){
  system.time(resflow<-helper_call_FlowSOM40(dataF[[i]][outliersWalkP[[i]]!=0,],  marker_cols = marker_cols[[i]], numC=true_n_clus[[i]])); gc()
  ...
  system.time(resflow<-helper_call_FlowSOM40(dataF[[i]][frankv(outl_resld[[i]]$lof[,2], ties.method = 'random', order=-1) > sum(outliersWalkP[[i]]==0),],  marker_cols = marker_cols[[i]], numC=true_n_clus[[i]]))
...
xres<-xtable(PerfMeasuresAllsets,  caption = "Tests results with Phenograph", digits=4, display=c('s', 's', 'fg','fg','fg','fg', 'fg'), include.rownames=FALSE, align = c("|c|c|c|c|c|c|c|"), label = "Phenograph_Levine32")
