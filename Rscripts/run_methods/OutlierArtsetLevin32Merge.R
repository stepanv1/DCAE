...
print(system.time(for(iteration in 2:iterations){
  ...
  #not nulify 
  COlist<-mclapply(1:length(clus_assignM), 
                   function(x)  unlist(lapply(1:num_clus, function(y) ifelse(CO[x,y]<0.5 & clus_assign[x]!=y, 0, CO[x,y] ) )), mc.cores=40) 
  CO <- do.call(rbind, COlist)
...
table(lblsT)
save(Assign,ScoreLst, file=paste0(DATA_DIR, '/FULLartLevin32Scores150.RData'))
