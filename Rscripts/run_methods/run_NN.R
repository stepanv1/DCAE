...
for (i in 1:1000) {
  ...
  dlmodel <- h2o.deeplearning(x = 1:10,  # column numbers for predictors
                              y = 11,   # column number for label
                              training_frame = FlowCAP1.hex.Train, # data in H2O format
                              validation_frame = FlowCAP1.hex.Valid,
                              activation  = "Tanh",
                              input_dropout_ratio =rand_input_dropout, # % of inputs dropout
                              hidden = rand_hidden, adaptive_rate =F,
                              momentum_start =  0.5, momentum_stable = 0.9, l1 = rand_l1,
                              epochs = 10,   loss = "Quadratic")
  models <- c(models, dlmodel)
}
plot(models[[1]])

best_err <- models[[1]]@model$validation_metrics@metrics$mean_per_class_error

for (i in 1:length(models)) {
  err <- models[[i]]@model$validation_metrics@metrics$mean_per_class_error
  if (err < best_err) {
    best_err <- err
    best_model <- models[[i]]
  }
...
dataset.test<-as.data.frame(data$FlowCAP_ND[[12]])
unassigned <- is.na(dataset.test[,'label'])
dataset.test <- dataset.test[!unassigned,]
dataset.test[,!(colnames(dataset.test) %in% 'label')]<-tanh(scale(dataset.test[,!(colnames(dataset.test) %in% 'label')], center=T))

FlowCAP1.hex.Test = as.h2o(dataset.test[,1:10])
system.time(predictions <- h2o.predict(best_model, FlowCAP1.hex.Test))
