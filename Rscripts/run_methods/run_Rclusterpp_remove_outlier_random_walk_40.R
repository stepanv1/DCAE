...
HierarchicalClus<- vector("list", length(files_truth))
HierarchicalClusOutlier<- vector("list", length(files_truth))
#########################################################################################################
...
for (i in 1:length(files)){
  ...
  system.time({
    ...
    out <- Rclusterpp.hclust( data[[i]], method = method[[i]] )
    resflow <- cutree(out, k = 40)})
  HierarchicalClus[[i]]<-out
  ...
  Pasym_w<-EnhanceDensity(P=(asym_wn), V=rep(1,  dim(asym_wn)[1])/sum(rep(1,  dim(asym_wn)[1])), smooth=FALSE, debug=TRUE, maxit=10000, alpha=1, eps=5e-6); gc()
  ...
  system.time(
    {
      Rclusterpp.setThreads(10)
      out <- Rclusterpp.hclust( data[[i]][outliersWalk[[i]], ], method = method[[i]] )
      cl_res_walk <- cutree(out, k = 40)})
  HierarchicalClusOutlier[[i]]<-out
  ...
  print('starting clustering')
  print(paste0('set # ', i))
  #system.time(cl_resi<<-flowMeans(data[[i]][IDX , ])@Label) ;gc()
...
setwd("/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/Rscripts/run_methods")
save(outliers2Step, outliersWalk, outliersWalkP, clus_assign, clus_assign_2Step, clus_assign_walk, HierarchicalClusOutlier, HierarchicalClus, file=paste0(RES_DIR, '/', CALC_NAME, '.RData'))
...
# Package ‘HighDimOut’ -improved LOF, Feature Bagging for Outlier Detection 
library(ldbod)
outl_resld <- vector("list", length(files_truth))
#scale(data[[i]])
system.time(
for(i in 1:length(outl_resld)){
      outl_resld[[i]]<-ldbod((data[[i]]), k = c(15,30,45), nsub = nrow(data[[i]]),  ldf.param = c(h = 1, c = 0.1), rkof.param = c(alpha = 1, C = 1, sig2 = 1), lpdf.param = c(cov.type = "full", sigma2 = 1e-05, tmax = 1, v = 1),  treetype = "kd", searchtype = "standard", eps = 0, scale.data = TRUE)}
)

save(outl_resld, file=paste0(RES_DIR, '/',  'lbod_results', '.RData'))
