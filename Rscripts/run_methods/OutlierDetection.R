...
g <-erdos.renyi.game(100000, 5*10^(-5))
g <- barabasi.game(10000, power=1)
layout <- layout.fruchterman.reingold(g)
plot(g, layout=layout, vertex.size=2,
     vertex.label=NA, edge.arrow.size=.2)
plot(g)
system.time(z<-cluster_walktrap(g))
table(membership(z))
gc()
system.time(zz<-cluster_louvain(g))
table(membership(zz))
system.time(zz<-cluster_infomap(g))
table(membership(zz))
gs <-erdos.renyi.game(10, 2/3)
plot(g)
#g <- barabasi.game(5)
#g <- sample_islands(20, 15000, 5*10^(-2), 100)
sum(count_triangles(g))/3
#triangles(g)
count_motifs(g, size = 2)
gs_adj<-as_adjacency_matrix(gs, type = c("both"), attr = NULL,
...
plot(g)
pm <- cbind( c(.005, 0.001), c(0.001, 0.002) )
g <- sample_sbm(10000, pref.matrix=pm, block.sizes=c(3000,7000))
plot(degree_distribution(g))
#um(count_triangles(g))/3
...
A2<-g_adj * (g_adj %*% g_adj );gc() 
#A3<-g_adj * (g_adj %*% g_adj %*% g_adj);gc() 
Atot<-A2#+A3

#calculate nominator and denomenator of edge reweighting coefficient
sum_strength_mat<-g_adj * vert_weight; sum_strength_mat<-sum_strength_mat + t(sum_strength_mat);gc()
sum_ew_mat<-g_adj * (vert_weight/g_deg); sum_ew_mat<-sum_ew_mat + t(sum_ew_mat);gc()

#calculate re-weighting coefficient and adjacency matrix
sum_strength_mat <- as(sum_strength_mat, "dgTMatrix") #some unpleasantries with elementwise operations
sum_strength_mat@x<-1/sum_strength_mat@x
sum_strength_mat_inv <- as(sum_strength_mat, "dgCMatrix")

#degree based reweighting
sum_deg_mat <- g_adj*vert_degree; sum_deg_mat<-sum_deg_mat + t(sum_deg_mat);gc()
sum_deg_mat <- as(sum_deg_mat, "dgTMatrix") #some unpleasantries with elementwise operations
sum_deg_mat@x<-1/sum_deg_mat@x
sum_deg_mat_inv <- as(sum_deg_mat, "dgCMatrix")

#probability based reweighting

#rwc <- (Atot * (sum_ew_mat * sum_strength_mat_inv));gc()
rwc <- (Atot * (2* sum_deg_mat_inv));gc()
...
#
#############################################################################
gr_res <- mcReweight(g_w, addLoops = FALSE, expansion = 2, inflation = 2,  max.iter = 1, ESM = TRUE); gc()
gr_res[[1]]
gr_w<-gr_res[[2]] 
sort(colSums(gr_w),decreasing = T)
nnzero(gr_w)
