...
for (i in 1:length(outliersWalkP)){
  print('starting clustering after RW clean')
  print(paste0('set # ', i))
  system.time(resflow<-flowMeans(data[[i]][outliersWalkP[[i]]!=0,],  NumC = 40)@Label); gc()
  ...
  print('starting clustering after LOF clean')
  print(paste0('set # ', i))
  system.time(resflow<-flowMeans(data[[i]][frankv(outl_resld[[i]]$lof[,2], ties.method = 'random', order=-1) > sum(outliersWalkP[[i]]==0),], NumC = 40)@Label)
...
xres<-xtable(PerfMeasuresAllsets,  caption = "Tests results with flowMeans", digits=4, display=c('s', 's', 'fg','fg','fg','fg', 'fg'), include.rownames=FALSE, align = c("|c|c|c|c|c|c|c|"), label = "flowMeans_Levine32")
