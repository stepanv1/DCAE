...
i=4
ddM<-as.matrix(dist(data[[i]][sample(1:nrow(data[[i]]), 10000),], method='manhattan'))
dM=density(as.vector(ddM[ddM!=0]))
hist(ddM[ddM!=0],500, main='manhattan', prob=T)
ddH<-as.matrix(dist(data[[i]][sample(1:nrow(data[[i]]), 10000),]))
hist(ddH[ddH!=0],500, main='euclidian', prob=T)

#characteristic distance
maxd<-optimize(approxfun(dM$x,dM$y),interval=c(min(dM$x),max(dM$x)), maximum=T)$maximum
maxdd<-optimize(approxfun(dM$x[-1],diff(dM$y)),interval=c(min(dM$x),max(dM$x)), maximum=T)$maximum
chd<-maxdd+(maxd-maxdd)/2
...
files_truth <- list(
  Samusik_all = file.path("../../benchmark_data_sets", "Samusik_01.fcs") 
)
...
#####################################################################
#add the loneliness index and distance/kernel correction
#system.time(jaccard45[, distL1:=sum(abs(data[[4]][V1,]-data[[4]][V2,])), by = 1:nrow(jaccard45)])

#jaccard45 <- dirtable(jaccard45)
#jaccard45<-Loners(jaccard45)
#try add distance to reweight weight in adjacency matrix
jaccard45[, distWeight:=V3/distL1]
#dirname=paste0(RES_DIR, CALC_NAME)
#system(paste0('mkdir ', dirname))
#save(jaccard45, file=paste0(dirname, '/jaccard45.RData'))
#load( file=paste0(dirname, '/jaccard45.RData'))
#remove triangles from those with linkCount==2
#lc2Triangle<-jaccard45[linkCount==2 & recCount==2,  V2[2] %in% ind[V2[1],] & V2[1] %in% ind[V2[2],], by=V1]
#setnames(lc2Triangle, c('V1', 'isInTriangle'))
#notinTriV1<-lc2Triangle[isInTriangle==FALSE,]$V1
#
#olidx<-unique(jaccard45[linkCount<2 | (V1 %in% notinTriV1),V1])# Remove potential bridges

#next iteration of attemt to remove bridges
#On those with the reciptocity score 2 (link) we perfo
#lc2Triangle<-jaccard45[linkCount==2 & recCount==2,  .(sort(c(V2[1], V2[2]))[1], sort(c(V2[1], V2[2]))[2], V2[2] %in% ind[V2[1],] & V2[1] %in% ind[V2[2],]) & (intersect(ind[V2[1],], ind[V2[2],])==V1 & length(intersect(ind[V2[1],], ind[V2[2],]))==1), by=V1]                      
lc2Triangle<-jaccard45[linkCount==2 & recCount==2,  .(sort(c(V2[1], V2[2]))[1], sort(c(V2[1], V2[2]))[2], (length(intersect(ind[V2[1],], ind[V2[2],]))==1) & intersect(ind[V2[1],], ind[V2[2],])[1]==V1), by=V1] 
setnames(lc2Triangle,  c('V1','n1', 'n2', 'triangle')) 
linkRm<-lc2Triangle[triangle==TRUE,paste(n1, n2) ]
                                                                                                                 #remove bridges nased on modularity model

#1. Caclulate probabilшty that two points are connected given that 
# they have shared neighbor, p

#create a table with all connections
#caclulate jaccard neighbors
jn<-as.data.table(matrix(0, nrow=length(unique(jaccard45[,V1])), ncol=45*2+1))
setnames(jn, paste0('Y', 1:(45*2+1)))
gc()
invisible(lapply((1:nrow(jaccard45))[1:10], function(x) {f<-jaccard45[x, V1]; jn[f, Y91:=Y91+1]; id<- jn[f, Y91]; jn[f,id]<-jaccard45[x, V2] }))
gc()

jaccard45[, {jn[V1,Y91:=Y91+1]; jn[V1, Y91]<-V2}, by = V1]

invisible(jaccard45[1:200, {l<-.N; m<-.SD; jn[V1,Y91:=Y91+l]; ind=V1;  jn[ind, 1:l] <- m[,V2]}, by = V1])
gc()
lapply(1:nrow(jaccard45))[1:1000] function(x) )


conns<-jaccard45[ , jaccard45[jaccard45[1]==V1, V2] , by = 1:nrow(jaccard45)]
zzz=jaccard45[, length(intersect(jaccard45[], jaccard45[]))>=1, by = 1:nrow(jaccard45)]


#zzz=jaccard45[1:100, lapply(1:(dim(combn(V2))[2]), function(x) {a=combn(V2);
#length(intersect(ind[a[1,x],], ind[a[2,x],]))}), by=V1]



                                                                                                  

olidx<-unique(jaccard45[linkCount<=0,V1])# Remove potential bridges
#olidx<-setdiff(unique(jaccard45[,V1]), unique(jaccard45[recCount==1,V1]))
...
#graphTbl<-jaccard45[recCount==2, c(1,2,3,9), ]

length(unique(graphTbl[,V1]))
graphTbl<-graphTbl[V3!=0,]
graphTbl<-graphTbl[order(V1)]
graphTbl<-graphTbl[!(EdgeKey %in% linkRm),]
setnames(graphTbl,c('V1', 'V2', 'V3'),  c('from', 'to', 'weight'))
gr<-graph_from_data_frame(as.data.frame(jaccard45[,1:3]), directed = F, vertices = NULL);gc()
E(gr)$weight=as.numeric(as.data.frame(jaccard45)[,3]);gc()
#E(gr)$weight=as.numeric((graphTbl[,distWeight]))
gr <- simplify(gr, remove.loops=FALSE, edge.attr.comb=list(weight="sum"));gc()
#hist(E(gr)$weight,500)


g_w=as_adjacency_matrix(gr, attr="weight", type = c("both"), 
...
#
#############################################################################
gr_res <- mcReweight(g_w, addLoops = F, expansion = 2, inflation = 2,  max.iter = 1, ESM = TRUE); gc()



#calculate local topology parameters of the graph
trian=count_triangles(gr)
Ntrian=sum(trian)/3
g_deg<-degree(gr)
deg_list= unlist(unique(g_deg))
#vertices of triangles
triVert<-as.numeric(triangles(gr));gc()
...
edgeIDs<- get.edge.ids(gr, vp=vert_ID, directed = F, error = FALSE, multi = FALSE)
edgeWe<-edge_attr(gr, 'weight', index = edgeIDs);gc()
...
#save(gr, file=paste0(dirname, '/L1_gr45ver2.RData'))
...
load(file=paste0(dirname, '/L1_com45ver2.RData'))
#save(com, file=paste0(dirname, '/L1_com45ver2.RData'))

clus_core<-membership(com)
clus<-rep(NaN, 841644)
clus[!((1:length(clus_truth)) %in% olidx)] <- clus_core
#IDX_<-unique(unlist(graphTbl[,1:2]))
#clus[IDX_]<-clus_core
#clus[(1:841644)[(1:841644) %in% intersect(setdiff(1:841644,isna), IDX_)]] <- clus_core
...
save(rb, file=paste0(RES_DIR, 'KdependencySamusik_840k/','RandomForestSamusikRboristBalancever2.RData'))
system.time(pred <- predict(rb, data[[4]][((1:nrow(data[[4]])) %in% olidx),]))
yPred <- pred$yPred
clusForest<-clus
clusForest[((1:length(clus_truth)) %in% olidx)]<-yPred

#write.table(clusForest, file =paste0(RES_DIR, 'KdependencySamusik_840k/', 'clusForestBalancever2.txt'), row.names = FALSE, quote = FALSE, sep = "\t")
clusForest<-unlist(read.table(paste0(RES_DIR, 'KdependencySamusik_840k/', 'clusForestBalancever2.txt'), header = T, stringsAsFactors = FALSE))
...
clus2 <- read.table(paste0(RES_DIR, 'KdependencySamusik_840k/', 'clusPG.txt'), header = F, stringsAsFactors = FALSE)[,1][-1]
helper_match_evaluate_multiple(clus2, clus_truth)

library("cluster")
IDX=sample(1:length(as.integer(unlist(clusForest))),20000)
sil <- silhouette(as.integer(clusForest[IDX]), dist(data[[4]][IDX,],  method = "euclidean"))
sil2 <- silhouette(as.integer(clus2[IDX]), dist(data[[4]][IDX,],  method = "euclidean"))
png(file=paste0(RES_DIR, 'KdependencySamusik_840k/', 'euclsil1.png'), width = 2000,height = 2000)
plot(sil, main ="proto2")
dev.off()

png(file=paste0(RES_DIR, 'KdependencySamusik_840k/', 'euclsil2.png'), width = 2000,height = 2000)
plot(sil2, main ="proto2_PhenClus")
dev.off()

library(scales)
hist(sil2[,3],100,  col = alpha('red', 0.5))
hist(sil[,3],100, col = alpha('green', 0.5), add = T)
