...
#from large samusik 39 dim data
########################################################
# S. Grinek 28.06.18
...
seed<-set.seed(12345)
#CALC_NAME = 'DELETE'
CALC_NAME<-'ArtDataOutliersLevine32'
CALC_NAMEold<-'Louvain_L1'
k=30
...
clus_truthL<-lapply(clus_truth, function(x) x[,1])
clus_truthL<-lapply(clus_truthL, function(x) ifelse(is.na(x),1000, x ))
...
load(file=paste0(RES_DIR, '/', CALC_NAMEold, '.RData'))
i=1
#extracting just one individual
#clus_truth[[i]]<-clus_truth[[i]]$X1[data[[i]][,'individual']==1]
#clus_truthL[[i]]<-clus_truthL[[i]][data[[i]][,'individual']==1]
#data[[i]]<-data[[i]][data[[i]][,'individual']==1, ]
data[[i]]<-data[[i]][,1:32]
# find reference subdimensions
source('/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/Rscripts/helpers/helper_find_subdimensions.R')
data[[i]][!is.na(clus_truth[[i]]), ]=apply(data[[i]][!is.na(clus_truth[[i]]), ], 2, function(x) ifelse(x<0, 0,x))
...
cln=1
#corabs<-(abs(cor(data[[i]][clus_assign[[i]]==cln, (1:32)[ref_subs[[i]]$subdim[cln,]] ], method='spearman')))
...
library(beanplot)
i=1
rs<-ref_subs[[i]]$subdim
dm<-ncol(rs)
tcl <- clus_truth[[i]][!is.na(clus_truth[[i]])]
dataSam <- data[[i]][!is.na(clus_truth[[i]]), ]
#dataSam <- apply(dataSam, 2, function(x) x-min(x))
dataSam=apply(dataSam, 2, function(x) ifelse(x<0, 0,x))
#beanplot(as.data.frame(dataSam[tcl==5,]), log="", what = c(T, T, F, F))
boxplot(dataSam[tcl==2,])
hist(dataSam[tcl==2,1],200)
#kde(dataSam[tcl==2,1], h=hpi(dataSam[tcl==2,1]))

tl=clus_truthL[[i]][clus_truthL[[1]]!=1000]
...
dataSamPositive <- apply(dataSam, 2, function(x) ifelse(x<=0,0.00001,x))
Fullcorellations <- lapply(1:nclus, function(x) cor(log(dataSamPositive[tcl==x, ])))
rm(dataSamPositive)
...
#generate noise
#withhod data at zero and add it back after density estimates 
zeros<-apply(dataSam, 2, function(x) ifelse(x==0, F,T) )
ncols=ncol(dataSam)
system.time(noisy_densities <- mclapply(1:nclus, function(z) {
  fhat <- lapply((1:ncols)[!rs[z,]], function(x){
    y=dataSam[tcl==z, x][zeros[tcl==z, x]]
    hat <-kde(x=y, h=hpi(y), positive=T)})
  return(fhat)}, mc.cores=10))

zzz=noisy_densities[[1]][[1]]
...
subdimCorellations <- lapply(1:nclus, function(x) {
  Fullcorellations[[x]][!rs[x,], !rs[x,]]<-0
  return(Fullcorellations[[x]])})
...
# we use lognornal distribution to approxinmate relevant dimensions
library('MethylCapSig')
artData <- lapply(1:nclus, function(x) {
  Y = matrix(0, ncol=dm, nrow=populations[x])
  Y[, rs[x,]]=mvlognormal(n=populations[x],  Mu = subdimMeans[[x]][rs[x,]], Sigma = diag(subdimCovariances[[x]][rs[x,], rs[x,]]), R =  subdimCorellations[[x]][rs[x,], rs[x,]]) 
  return(Y)})
#old, multivariate normal
#artData <- lapply(1:nclus, function(x) {
#  Y = matrix(0, ncol=dm, nrow=populations[x])
...
source('/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/Rscripts/run_methods/helperDiscretContMix.R')
#zeros<-apply(dataSam, 2, function(x) ifelse(x==0, F,T) )
system.time(pAll <- mclapply(1:nclus, function(z) {
    p<- lapply((1:ncols)[!rs[z,]], function(x){
    y=sum(!zeros[tcl==z, x])/length(zeros[tcl==z, x])
    })
  return(p)}, mc.cores=10))

artData <- lapply(1:nclus, function(z) {
  artData[[z]][, !rs[z,]] <- matrix(unlist(lapply(1:sum(!rs[z,]), function(x) helper_DiscretContMix(n=populations[z],p=pAll[[z]][[x]], noisy_densities[[z]][[x]]))),
                                   nrow=populations[z], ncol = sum(!rs[z,]), byrow=F)
  return(as.data.table(artData[[z]]))})


sum(unlist(lapply(artData, function(x) sum(x==0))))
#squeez outliers
realMax=max(dataSam)


artData<-lapply(artData, function(x){apply(x, 2, function(z) ifelse(z>realMax, realMax/(1+exp(-z)),z))
})
artData<-lapply(artData, function(x) as.data.table(x))

#putting zeros back
#artData<-artData*zeros


cln=3

boxplot(as.data.frame(artData[[cln]])[, rs[cln,]], log = "", main='art data')
boxplot(as.data.frame(artData[[cln]])[, !rs[cln,]], log = "",   main='art data')
boxplot(as.data.frame(dataSam[tl == cln,])[, rs[cln,]], log = "",  main='real data')
boxplot(as.data.frame(dataSam[tl == cln,])[, !rs[cln,]], log = "",  main='real data')

heatmap.2(cor(as.data.frame(artData[[cln]])[, rs[cln,]]))
heatmap.2(cor(as.data.frame(dataSam[tl == cln,])[, rs[cln,]]))


aFrame<-rbindlist(artData)

aFrame <- as.matrix(aFrame)
colnames(aFrame)<-colnames(dataSam)
lbls <- unlist(lapply(1:nclus, function(z) rep(z,populations[z])))


heatmap.2(cor(as.data.frame(dataSam[tl == cln,])[, rs[cln,]]), main='real')
heatmap.2(cor(as.data.frame(aFrame[lbls == cln,])[, rs[cln,]]), main='art')

boxplot(as.data.frame(dataSam[tl == cln,])[, !rs[cln,]], log = "")
boxplot(as.data.frame(aFrame[tl == cln,])[, !rs[cln,]], log = "")

boxplot(as.data.frame(aFrame[lbls == cln,])[, rs[cln,]], log = "", main='art data')
boxplot(as.data.frame(aFrame[lbls == cln,])[, !rs[cln,]], log = "",   main='art data')
boxplot(as.data.frame(dataSam[tl == cln,])[, rs[cln,]], log = "",  main='real data')
boxplot(as.data.frame(dataSam[tl == cln,])[, !rs[cln,]], log = "",  main='real data')
...
boxplot(as.data.frame(dataSam[tl == cln,])[, rs[cln,]], log = "")
boxplot(as.data.frame(aFrame[tl == cln,])[, rs[cln,]], log = "")
beanplot(as.data.frame(dataSam[tl == cln,])[, rs[cln,]], log = "", what = c(T, T, F, F))
beanplot(as.data.frame(aFrame[tl == cln,])[, rs[cln,]], log = "", what = c(T, T, F, F))


#aFrame<-apply(aFrame, 2, function(x) ifelse(x<0,0,x))

#retoring proportion of zeroes


save(aFrame, lbls,  file=paste0(DATA_DIR, '/artLevine32FLOCK.RData'))
