...
load(file=paste0(dirname, '/L1_gr45.RData'))
...
mome()
######################################################################
#Now, reassign the rest with random forest ###########################
library(ranger)
dataset.train=as.data.frame(cbind(data[[4]][!((1:nrow(data[[4]])) %in% olidx),], clus_core))
system.time(rf <- ranger(clus_core ~ ., data = dataset.train, write.forest = T, num.threads = 10))
save(rf, file=paste0(RES_DIR,'RandomForestSamusik.RData'))
#Samusik set
