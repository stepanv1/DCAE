# based on: https://rdrr.io/github/yishaishimoni/phenoClust/src/R/louvain.R
# modified with the aim to provide fixed number of clusters and to run on sparse matrices
...
louvain <- function(G, C=1:ncol(G), maxreps=100, verbose=TRUE, num.clus=1000){
  ...
  for (i in 1:maxreps){
    ...
    if (L$newQ > Q){
      ...
      #stop, if reached required number of clusteres
      nClust <- length(unique(C))
      if (nClust <= num.clus) break
