...
print(system.time(clus_assign<-helper_call_FlowSOM40(DFtoFF(as.data.frame(data)), marker_cols=1:dim(data)[2])))
# now we need to remove small clusters for our algorithm to work 
# we temporarily assign them to outler set and reassign later
# at the stage of  application of random forest algorithm
tblAssign<-table(clus_assign)
bigCluster<- unlist(lapply(clus_assign, function(x) ifelse(tblAssign[x]<8,FALSE, TRUE)))
bData<-data[bigCluster,]
bClus<-as.integer(as.factor(clus_assign[bigCluster]))


clus_assign0 <- clus_assign
#clus_assign <- clus_assign0
helper_match_evaluate_multiple(bClus, as.numeric(as.factor(lblsT[bigCluster])))
#save(clus_assign, file=paste0(DATA_DIR, '/artLevin32Clus_assign.RData'))
#load( file=paste0(DATA_DIR, '/artLevin32Clus_assign.RData'))
num_clus<-length(unique(bClus))

rsClus<-helper_find_subdimensions(bData, bClus, skewCut=1)
heatmap.2(as.matrix(ifelse(rsClus$subdim, 1,0)))

#merging identical signatures
system.time(clus_assignM<-helper_merge_signatures(rsClus$subdim, bClus,  bData,  mc.cores=10))
rsClus<-helper_find_subdimensions(bData, clus_assignM, skewCut=1)
heatmap.2(as.matrix(ifelse(rsClus$subdim, 1,0)))
table(clus_assignM)
num_clus<-length(table(clus_assignM))

helper_match_evaluate_multiple(clus_assign, as.numeric(as.factor(lblsT)))
helper_match_evaluate_multiple(clus_assignM, as.numeric(as.factor(lblsT[bigCluster])))
AMI(clus_assign, as.numeric(as.factor(lblsT)))
AMI(clus_assignM, as.numeric(as.factor(lblsT[bigCluster])))



GO<-helper_global_outliers_Continuous_Sums(bData, rsClus$subdim, clus_assignM, nbins=nbins, mc.cores=20)
print(system.time(LO<-mclapply(1:num_clus, function(i)  {rs<- clus_assignM==i; helper_local_outliersLOF2(bData[,rsClus$subdim[i,]], ref_subset = rs, k=30)}, mc.cores=10)))
...
idxOutlClus<-unlist(lapply(1:sum(bigCluster), function(i) ifelse(score[i]<1, F, T )))
...
dataNew<-bData[!outliers,]
dim(dataNew)

print(system.time(lblsNew<-helper_call_FlowSOM40(DFtoFF(as.data.frame(dataNew)), marker_cols=1:dim(data)[2])))
tblAssign2 <- table(lblsNew)
table(lblsNew)
bigCluster2<- unlist(lapply(lblsNew, function(x) ifelse(tblAssign2[x]<8,FALSE, TRUE)))
bData<-dataNew[bigCluster2,]
bClus<-as.integer(as.factor(lblsNew[bigCluster2]))
...
table(lblsT)  
save(Assign,ScoreLst, file=paste0(DATA_DIR, '/artLevincoresALLclusters.RData'))
...
cor((sumScores/max(sumScores))[2:15],AMIlist[1:(iterations-1)], method='spearman')
plot((sumScores/max(sumScores))[2:15], AMIlist[1:(iterations-1)])
