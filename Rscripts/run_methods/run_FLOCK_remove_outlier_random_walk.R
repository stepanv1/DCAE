...
for (i in 1:length(files)){
  ...
  system.time(resflow<-helper_call_Flock(data[[i]], FLOCKDIR = '/home/sgrinek/bin/FLOCK')$Population)
  table(resflow)
  ...
  #run on modified set
  source('/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/Rscripts/helpers/helper_call_FLOCK.R')
  system.time(cl_res_walk<-helper_call_Flock(data[[i]][outliersWalk[[i]], ], FLOCKDIR = '/home/sgrinek/bin/FLOCK')$Population) ;gc()
  table(cl_res_walk)
  ...
  system.time(cl_resi<-helper_call_Flock(data = data[[i]][IDX , ], FLOCKDIR = '/home/sgrinek/bin/FLOCK')$Population) ;gc()
