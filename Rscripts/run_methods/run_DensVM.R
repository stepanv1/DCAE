...
for (i in ix_subsample) {
  if (!is_FlowCAP[i]) {
    set.seed(123)
    IDX<-  data.frame(idx=sample(1:nrow(data[[i]]), n_sub[i])) 
    data[[i]] <- data[[i]][IDX[,1], ]
    ...
    files_true_labels_i <- paste0("../../results/auto/DensVM/true_labels_DensVM_", 
                                  names(data)[i], ".txt")
    fidx <- files_true_labels_i <- paste0("../../results/auto/DensVM/IDXsample_DensVM_", 
                                          names(data)[i], ".txt")
    
      write.table(true_labels_i, file = files_true_labels_i, row.names = FALSE, quote = FALSE, sep = "\t")
      write.table(IDX, file = fidx, row.names = FALSE, quote = FALSE, sep = "\t")  
