...
library(Matrix)
library(data.table)
source('/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/Rscripts/helpers/helper_2d_hist.R')
CALC_ID = 'Artificial_data_PHENOGRAPH'
RES_DIR <- '../../results/outlier_compare'
chunks2<- function(n, nChunks) {split(1:n, ceiling(seq_along(1:n)/(n/nChunks)))}

i=1

clus_set<-vector('list', 50)
for(i in 1:length(clus_set)){  
  clus_set[[i]]<-genRandomClust(numClust=25,
                           sepVal=-0.1,
                           numNonNoisy=15,
                           numNoisy=0,
                           numOutlier=0.05,
...
save(clus_set, file=paste0(RES_DIR, '/', CALC_ID, '_clus_set_', 'RData'))

relations<-vector('list', 50)
cl_coord<-vector('list', 50)
for (i in 1:length(clus_set)){ 
  cl_coord[[i]]=clus_set[[i]]$datList$test_1
  lbls=clus_set[[i]]$memList$test_1
  table(lbls)
  ...
  gc()
  k=30
  m2<-find_neighbors(cl_coord[[i]], k=k+1);
  neighborMatrix <- (m2$nn.idx)[,-1]
  system.time(links <- cytofkit:::jaccard_coeff(m2$nn.idx))
  gc()
  links <- links[links[,1]>0, ]
  relations[[i]] <- as.data.frame(links)
  #save(relations, file='../../results/relationsSamusik_01.RData')
  colnames(relations[[i]])<- c("from","to","weight")
  relations[[i]]<-as.data.table(relations[[i]])
#relations<-relations[from!=to, ]# remove self-loops
}
save(relations, cl_coord, file=paste0(RES_DIR, '/', CALC_ID, '_relations_', 'RData'))

n_set=2
ix<-c(10,12)
lbls=clus_set[[n_set]]$memList$test_1
cl_labels<-c(22,25)
x1<-cl_coord[[n_set]][,ix[1]]; x2<-cl_coord[[n_set]][,ix[2]]
hist_2d(x1,x2, n=150)
x1<-cl_coord[[n_set]][lbls %in% cl_labels,][,ix[1]]; x2<-cl_coord[[n_set]][lbls %in% cl_labels,][,ix[2]]
hist_2d(x1,x2, n=150)
