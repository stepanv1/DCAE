...
# using random walk. FlowSOM at k=40 with the numberr of cluster given by 
# manual gating.
...
for (i in 1:length(files)){
  ...
  #################################################################################
  system.time(resflow<-helper_call_FlowSOM40(dataF[[i]], marker_cols = marker_cols[[i]], numC=40))
  table(resflow)
  ...
  #run on modified set
  system.time(cl_res_walk<-helper_call_FlowSOM40(dataF[[i]], subset=outliersWalk[[i]], marker_cols = marker_cols[[i]], numC=40)); gc()  
  table(cl_res_walk)
  ...
  system.time(cl_resi<-helper_call_FlowSOM40(dataF[[i]], subset=IDX, marker_cols = marker_cols[[i]], numC=40)); gc()
