...
# using random walk. Siluette index.
...
JAC_DIR = '/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/results/kk15L2cosine'
files <- list(
  Levine_32dim = file.path(JAC_DIR, "j15Levine_32dim.txt"), 
  Levine_13dim = file.path(JAC_DIR, "j15Levine_13dim.txt"), 
  Samusik_01   = file.path(JAC_DIR, "j15Samusik_01.txt"), 
  Samusik_all  = file.path(JAC_DIR, "j15Samusik_all.txt")
  #Nilsson_rare = file.path(DATA_DIR, "Nilsson_rare.fcs"), 
  #Mosmann_rare = file.path(DATA_DIR, "Mosmann_rare.fcs"), 
  #FlowCAP_ND   = file.path(DATA_DIR, "FlowCAP_ND.fcs"), 
  #FlowCAP_WNV  = file.path(DATA_DIR, "FlowCAP_WNV.fcs")
...
for (i in 1:length(files)){
  ...
  resflow <- louvain_multiple_runs(gL, num.run = num.louvain.run);
  gc()
  table(resflow)
  clus_assign[[i]] <- resflow
  
  # use random walk to isolated, weakly connect nodes  #############################
  ##################################################################################
  asym_wn<-norm_mat(asym_w)
  Pasym_w<-EnhanceDensity(P=(asym_wn), V=rep(1,  dim(asym_wn)[1])/sum(rep(1,  dim(asym_wn)[1])), smooth=FALSE, debug=TRUE, maxit=5000, alpha=1, eps=5e-06)
  ...
  indg<-induced_subgraph(gL, (1:gorder(gL))[outliersWalk[[i]]])
  
  #cl_res<-cluster_louvain(g)
  cl_res_walk<-louvain_multiple_runs(indg, num.run = num.louvain.run);gc()
  
  
  clus_assign_walk[[i]]=(cl_res_walk)
  
  #tracking the evolution of probability distribution
  #################################################################################################
  #IDXo<-sample((1:dim(asym_wn)[1])[Pasym_w==0], 400); IDXi <- sample((1:dim(asym_wn)[1])[Pasym_w!=0], 400)
  #Ptrace<-EnhanceDensityTrace(P=(asym_wn), V=rep(1,  dim(asym_wn)[1])/sum(rep(1,  dim(asym_wn)[1])), IDX=c(IDXo, IDXi), smooth=FALSE, debug=TRUE, maxit=5000, alpha=1, eps=1e-16)
  ...
  #matplot(log(t(Ptrace$Piter[,])+0), pch='.', col = ifelse((1:800)>=401, 'red', 'black'), type = "l")
  #matplot(log(t(Ptrace$Piter[1:400,])+0), pch='.', type = "l", main='Outliers')
  #matplot(log(t(Ptrace$Piter[401:800,4000:5000])+0), pch='.', type = "l",  main='Core points')
  #matplot((t(Ptrace$Piter[,])+0), pch='.', col = ifelse((1:800)>=401, 'red', 'black'), type = "l")
  
  #logP<-as.numeric(log(Ptrace$Piter[1:800,516]))
  
  #it is confirmed that tragectory is very differnt for points with zero Ptrace$fin
  #louvain_multiple_runs(gL, num.run = num.louvain.run);gc()
   ##########################################################################
  #outlier removal with 2 step walk
  ...
  g2 <-graph_from_adjacency_matrix((asym_w+t(asym_w))/2, mode =  "undirected", weighted = TRUE, diag = F,                            add.colnames = NULL, add.rownames = NA); gc()
  ...
  #cl_res<-cluster_louvain(g)
  cl_resi<-louvain_multiple_runs(indg, num.run = num.louvain.run);gc()
  ...
  helper_match_evaluate_multiple(resflow, lbls)
}
save(outliers2Step, outliersWalkP, outliersWalk, outliersWalkP, clus_assign, clus_assign_2Step, clus_assign_walk, file=paste0(RES_DIR, '/', CALC_NAME, '.RData'))

load(file=paste0(RES_DIR, '/', CALC_NAME, '.RData'))
###############################################################################################
#assignment of outliers to clusters and evaluation
#using silhouette and F1 measure. Tested algorithm is run
...
library('flexclust')
hist(dist2(data[[i]][io,], data[[i]][ii,], method = "euclidean", p=2),500); gc()

idxs <- sample(length(clus_assign[[i]]), 20000)
sind1<-silhouette(clus_assign[[i]][idxs], dist(scale(data[[i]][idxs,]), method = "manhattan")); gc()
plot(sind1)
median(sind1[ ,3])
dbs1<-dbs(scale(data[[i]]), clusters = clus_assign[[i]], prior=as.vector(table( clus_assign[[i]])/sum(table( clus_assign[[i]]))))
median(dbs1@dbs)

sind2<-silhouette(RFassignWalk[[i]][ii], dist(scale(data[[i]][ii,]), method = "manhattan")); gc()
plot(sind2)
median(sind2[ ,3])
dbs2<-dbs(scale(data[[i]]), clusters = RFassignWalk[[i]], prior=as.vector(table( RFassignWalk[[i]])/sum(table( RFassignWalk[[i]]))))
...
# in a large cluster in HD
#
i=4 
table(clus_assign[[i]])
IDXout<-which(!(1:dim(data[[i]])[1] %in% outliersWalk[[i]]))
ic<-(1:nrow(data[[i]]))[clus_assign[[i]]==9]
kpdfALL<-kepdf(scale(data[[i]][ic,]),  kernel = "t7")
median(kpdfALL@estimate)
pdfOUT<-kepdf(scale(data[[i]][ic,]), x= scale(data[[i]][IDXout,]) , kernel = "t7")
median(pdfOUT@estimate)
beep(8)

#testing package RLOF
