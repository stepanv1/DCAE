...
sum(colSums(gr_w)==0)
table(lbls)
plot(diag(gr_res$Equilibrium.state.matrix), pch='.')
...
Ptrace<-EnhanceDensityTrace(P=norm_mat(gr_res$Equilibrium.state.matrix - Diagonal(x = diag(gr_res$Equilibrium.state.matrix))), V=rep(1,  dim(asym_wn)[1])/sum(rep(1,  dim(asym_wn)[1])), IDX=c(IDXo, IDXi), smooth=FALSE, debug=TRUE, maxit=5000, alpha=1, eps=1e-20)
