...
data0 <-read.FCS("/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/Samusik_all.fcs", 
                 transformation = FALSE, truncate_max_range = FALSE)@exprs
lblsT=data0[,54]
aFrame<-as.matrix(data0[,9:47])
aFrame[aFrame<0]<-0
...
print(system.time({g<-create_snnk_graph_approx(data, k=30, metric='L2', mc.cores=5);
...
save(clus_assign, file=paste0(DATA_DIR, '/Sam39Clus_assign.RData'))
...
COlist<-mclapply(1:length(clus_assignM), 
                 function(x)  unlist(lapply(1:num_clus, function(y) ifelse(CO[x,y]<0.5 & clus_assignM[x]!=y, 0, CO[x,y] ) )), mc.cores=20) 
