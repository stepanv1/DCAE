...
for (i in 1:length(outliersWalkP)){
  system.time(resflow<-helper_call_FlowSOM40(dataF[[i]][outliersWalkP[[i]]!=0,],  marker_cols = marker_cols[[i]])); gc()
  ...
  system.time(resflow<-helper_call_FlowSOM40(dataF[[i]][frankv(outl_resld[[i]]$lof[,2], ties.method = 'random', order=-1) > sum(outliersWalkP[[i]]==0),],  marker_cols = marker_cols[[i]]))
...
system.time(resSOD<-Func.SOD(data[[i]], k.nn=30, k.sel=30, alpha = 0.8))
#
###LOF and RW outliers united
################################################################
i=2
plot(-log10(outliersWalkP[[i]]+10^(-100)), outl_resld[[i]]$lof[,2], pch='.')
...
xres<-xtable(PerfMeasuresAllsets,  caption = "Tests results with FlowSOM", digits=4, display=c('s', 's', 'fg','fg','fg','fg', 'fg'), include.rownames=FALSE, align = c("|c|c|c|c|c|c|c|"), label = "FlowSOM")
