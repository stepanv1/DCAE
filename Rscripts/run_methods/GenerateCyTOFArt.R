...
cln=1
corabs<-(abs(cor(data[[i]][clus_assign[[i]]==cln, (1:39)[ref_subs[[i]]$subdim[cln,]] ][,], method='spearman')))
...
tl=clus_truthL[[4]][clus_truthL[[4]]!=1000]
...
heatmap.2(cor(as.data.frame(dataSam[tl == cln,])[, rs[cln,]]))
heatmap.2(cor(as.data.frame(aFrame[lbls == cln,])[, rs[cln,]]))
...
save(aFrame, lbls,  file=paste0(DATA_DIR, '/artSam.RData'))
