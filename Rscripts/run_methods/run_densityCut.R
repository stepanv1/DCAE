...
CALC_NAME<-gsub("[[:space:]]", "",paste0('densityCut_ ',Sys.time()))
...
for (i in ix_subsample){
    if (!is_FlowCAP[i] ) {
        ...
        files_true_labels_i <- paste0(c("../../results/auto/densityCut/true_labels_densityCut_", 
                                        "../../results/manual/densityCut/true_labels_densityCut_"), 
                                      names(data)[i], ".txt")
...
dirname=paste0("../../results/auto/densityCut/", CALC_NAME)
...
#print(sessionInfo())
#sink()
save(data, file=paste0(dirname,"/", 'densityCut.RData'))
