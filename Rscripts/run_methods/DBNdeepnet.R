...
#FlowCAP1.hex.Train = as.h2o(dataset.train)
#FlowCAP1.hex.Valid = as.h2o(dataset.valid)

system.time(model.dbn<-dbn.dnn.train(as.matrix(dataset[, !(colnames(dataset) %in% 'label')]), 
                                     data.matrix(as.vector(dataset[, 'label'])),
                        hidden =c(100, 100),
                        learningrate = 0.8, numepochs  = 50, batchsize = 100, hidden_dropout = 0.5,
                        visible_dropout = 0.2))
...
predictions_df <- nn.predict(model.dbn, as.vector(dataset.test[,!(colnames(dataset.test) %in% 'label')]))
nn.test(model.dbn, dataset.test[,!(colnames(dataset.test) %in% 'label')], as.vector(dataset.test[,'label']))

AMI(as.integer(predictions_df[,1]), as.integer(dataset.test[,'labels']), mc.cores=1)
