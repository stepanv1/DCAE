...
i=1; k=100
lbls=clus_truth[[i]]
conn = mclapply((1:nrow(data[[i]])), function(x) {
  l = lbls[unlist(nnind105[[i]][x,2:k])] 
  l = l[ifelse(is.na(l), F, T )]
  if (length(l)<2) return(0);
  sum((table(l)/length(l))^2)
  }, mc.cores=10); gc()
conn=unlist(conn)
hist(conn,20000, xlim=c(0,0.01))
hist(conn[],20)
sum(conn>0 & conn <1)
sum(conn>0.4 & conn <0.6)
hist(conn[lbls==6],20000)
hist(conn[!is.na(lbls)],20)
hist(conn[is.na(lbls)],20)
...
heatmap.2(as.matrix(ifelse(ref_subs[[2]],1,0)))
table(clus_assign[[1]])
##########################################################################
...
for(i in 1:2){
  ...
  rs<-ref_subs[[i]]
  cl<-clus_assign[[i]]
  system.time(GO<-helper_global_outliers_Uniform(d, rs, cl, nbins=5, mc.cores=10))
...
save( GOmat, LOmat, COmat, localisation, max_density,  file=paste0(RES_DIR,'/out_data1.RData'))
#load(file=paste0(RES_DIR,'/out_data1.RData'))
gc()
...
for(i in 1:2){
  ...
  X11();plot((clus_truth[[i]]+(rnorm(length(clus_truth[[i]]), sd=0.2)))[clust_clean], ncl2 +abs(rnorm(length(ncl2), sd=0.2)), pch = '.', col = alpha('red', 0.8), ylim=c(0,length(table(cl2))))
...
i=1
cl<-clus_assign[[i]];cl2<-cluster_assign1[[i]];clust_clean<-clust_out1[[i]]
...
for(i in 1:2){
  ...
  #  g<-create_snnk_graph_from_subset(nn.idx, k, clust_clean, mc.cores=5)$graph
  #} else {
  print(system.time(g<-create_snnk_graph(d[clust_clean,], k=10,  metric='L2')$graph))
  ...
  print(AMI(cl, clus_truth[[i]]))
  print(AMI(cl2, clus_truth[[i]][clust_clean]))
  print(table(cl, clus_truth[[i]]))
  print(table(cl2, clus_truth[[i]][clust_clean]))
  helper_match_evaluate_multiple_SweightedN(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_SweightedN(cl2, clus_truth[[i]][clust_clean])
  helper_match_evaluate_multiple_MoCu(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_MoCu(cl2, clus_truth[[i]][clust_clean])
  ...
  ##try random forest on this
  source('/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/Rscripts/helpers/helper_random_forest.R')
  for_pred <- helper_assign_outliers(bulk_data=d[clust_clean,], d[!clust_clean,], cl2)
  #check performance
  cl3<-clus_truth[[i]];cl3[clust_clean]<-cl2; cl3[!clust_clean]<-for_pred; 
  print(rescl<-helper_match_evaluate_multiple(cl, clus_truth[[i]]))
  print(rescl2<-helper_match_evaluate_multiple(cl3, clus_truth[[i]]))
  print(AMI(cl, clus_truth[[i]]))
  print(AMI(cl3, clus_truth[[i]]))
  print(table(cl, clus_truth[[i]]))
  print(table(cl3, clus_truth[[i]]))
  helper_match_evaluate_multiple_SweightedN(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_SweightedN(cl3, clus_truth[[i]])
  helper_match_evaluate_multiple_MoCu(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_MoCu(cl3, clus_truth[[i]])
  
  ##################################################################################
  #one last step remove most outlying points
  #1. new subdims, find new scores, find top outlying points, remove, calculate new
  # scores for cleaned cl3
  rs<-helper_find_subdimensions(data[[i]], cl3)$subdim
  heatmap.2(ifelse(rs, 1, 0))
  system.time(GO2<-helper_global_outliers_Uniform(d, rs, cl3, nbins=5, mc.cores=10))
  print(system.time(LO2<-mclapply(1:nrow(rs), function(x) helper_local_outliersLOF2(d[, rs[x,]],  cl3==x, k=30), mc.cores=10, mc.preschedule=F)))
  #combo, LO*G0
  GOm2 = Reduce(cbind, GO2) 
  LOm2 = Reduce(cbind, (lapply(LO2, function(x) x$prob))) 
  CO2=as.matrix(GOm2*LOm2)
  colnames(CO2)=1:nrow(rs)
  LD2 <- apply(CO2,1, function(x) ifelse(sum(x>0),sum((x/sum(x))^2), 0) )
  MD2 <- apply(CO2,1, function(x) max(x) )
  hist(LD2,200)
  hist(MD2,200)
  
  inter2<-(MD2<0.01 & LD2 < 0.3)
  sum(inter2)
  table(clus_truth[[i]][inter2])
  
  sum(table(clus_truth[[i]][inter2]))
  clust_clean2<-!inter2
  cl4<-cl3[clust_clean2]
  
  print(rescl<-helper_match_evaluate_multiple(cl, clus_truth[[i]]))
  print(rescl4<-helper_match_evaluate_multiple(cl4, clus_truth[[i]][clust_clean2]))
  print(AMI(cl, clus_truth[[i]]))
  print(AMI(cl4, clus_truth[[i]][clust_clean2]))
  print(table(cl, clus_truth[[i]]))
  print(table(cl4, clus_truth[[i]][clust_clean2]))
  helper_match_evaluate_multiple_SweightedN(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_SweightedN(cl4, clus_truth[[i]][clust_clean2])
  helper_match_evaluate_multiple_MoCu(cl, clus_truth[[i]])
  helper_match_evaluate_multiple_MoCu(cl4, clus_truth[[i]][clust_clean2])
