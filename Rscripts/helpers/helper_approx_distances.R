...
find_neighbors <- function(data,  k, metric ='L2', mc.cores=40){
  ...
  if (metric=='L2'){
    ...
    index$createIndex(dict(c('post', 0, 'efConstruction',10000,'M',64)), print_progress=F)
    ...
    if (msk!=0){
      ...
      nearestCorrected <- RANN::nn2(data, query, k, treetype = "kd", searchtype = "standard", eps=0)
...
find_neighbors_vec <- function(data, query, k, metric ='L2'){
  #euclidian metric
  if (metric=='L2'){
    nearest <- RANN::nn2(data, query=matrix(query, byrow=T, nrow=1), k+1, treetype = "bd", searchtype = "standard", eps=0)
  }
  if (metric=='L1'){
...
find_neighborsTarget <- function(target, data, k, metric ='L2', mc.cores=40){
  #euclidian metric
  #if (k>=nrow(data)) {k<-k-2}
  if (metric=='L2'){
    ...
    index$addDataPointBatch(data)
    index$createIndex(dict(c('post', 0)), print_progress=F)
    print((c('post', 2)))
    # query for the nearest neighbours of the first datapoint
    #ids, distances = index$knnQuery(data[0,], k=k)
    ...
    if (msk!=0){
      ...
      nearestCorrected <- RANN::nn2(data, query, k, treetype = "kd", searchtype = "standard", eps=0.00000)
      indxs[errRows,] <- nearestCorrected$nn.idx
      dists[errRows,] <- nearestCorrected$nn.dists
    }
