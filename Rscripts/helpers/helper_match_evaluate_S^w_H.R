...
helper_match_evaluate_multiple_SweightedH <- function(clus_algorithm, clus_truth) {
  ...
  pr_mat <- re_mat <- SweightedH_mat <- matrix(NA, nrow = length(tbl_algorithm), ncol = length(tbl_truth))
  ...
  for (i in 1:length(tbl_algorithm)) {
    for (j in 1:length(tbl_truth)) {
      ...
      SweightedH_ij <- weight_meet_i_j * (1 - H_i_j/log2(j_card)) * (1 - H_j_i/log2(i_card))
      #cat(c(SweightedH_ij, '\n'))
      if (SweightedH_ij == "NaN") SweightedH_ij <- 0
      
      pr_mat[i, j] <- precision_ij
      re_mat[i, j] <- recall_ij
      SweightedH_mat[i, j] <- SweightedH_ij
    }
  }
  
  # put back cluster labels (note some row names may be missing due to removal of unassigned cells)
  rownames(pr_mat) <- rownames(re_mat) <- rownames(SweightedH_mat) <- names(tbl_algorithm)
  colnames(pr_mat) <- colnames(re_mat) <- colnames(SweightedH_mat) <- names(tbl_truth)
  ...
  SweightedH_mat_trans <- t(SweightedH_mat)
  
  if (nrow(SweightedH_mat_trans) <= ncol(SweightedH_mat_trans)) {
    # if fewer (or equal no.) true populations than detected clusters, can match all true populations
    labels_matched <- clue::solve_LSAP(SweightedH_mat_trans, maximum = TRUE)
    # use row and column names since some labels may have been removed due to unassigned cells
    labels_matched <- as.numeric(colnames(SweightedH_mat_trans)[as.numeric(labels_matched)])
    names(labels_matched) <- rownames(SweightedH_mat_trans)
    ...
    labels_matched_flipped <- clue::solve_LSAP(SweightedH_mat, maximum = TRUE)
    # use row and column names since some labels may have been removed due to unassigned cells
    labels_matched_flipped <- as.numeric(rownames(SweightedH_mat_trans)[as.numeric(labels_matched_flipped)])
    names(labels_matched_flipped) <- rownames(SweightedH_mat)
    
    labels_matched <- rep(NA, ncol(SweightedH_mat))
    names(labels_matched) <- rownames(SweightedH_mat_trans)
  ...
  pr <- re <- SweightedH <- n_cells_matched <- rep(NA, ncol(SweightedH_mat))
  names(pr) <- names(re) <- names(SweightedH) <- names(n_cells_matched) <- names(labels_matched)
  
  for (i in 1:ncol(SweightedH_mat)) {
    ...
    SweightedH[i] <- ifelse(is.na(labels_matched[i]), 0, SweightedH_mat[as.character(labels_matched[i]), names(labels_matched)[i]])
  ...
  mean_pr <- mean(pr)
  mean_re <- mean(re)
  mean_SweightedH <- mean(SweightedH)
  total_SweightedH <- sum(SweightedH_mat, na.rm = TRUE)
  
  return(list(n_clus = n_clus, 
              pr = pr, 
              re = re, 
              SweightedH = SweightedH, 
              labels_matched = labels_matched, 
              n_cells_matched = n_cells_matched, 
              mean_pr = mean_pr, 
              mean_re = mean_re, 
              mean_SweightedH = mean_SweightedH,
              total_SweightedH = total_SweightedH,
              SweightedH_mat = SweightedH_mat))
}
