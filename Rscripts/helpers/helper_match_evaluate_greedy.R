...
helper_match_evaluate_greedy <- function(clus_algorithm, clus_truth) {
  ...
  if (nrow(F1_mat_trans) <= ncol(F1_mat_trans)) {
    ...
    labels_matched <- unlist(lapply(1:nrow(F1_mat_trans), function(x) which(F1_mat_trans[x,] == max(F1_mat_trans[x,]))))
    ...
    labels_matched_flipped <- unlist(lapply(1:nrow(F1_mat), function(x) which(F1_mat == max(F1_mat[x,]))))
    # use row and column names since some labels may have been removed due to unassigned cells
  ...
  mean_pr <- mean(pr)
  mean_re <- mean(re)
  mean_F1 <- sum((tbl_truth/sum(tbl_truth))*F1)
