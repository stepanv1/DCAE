...
helper_match_evaluate_multiple_RandInd <- function(clus_algorithm, clus_truth) {
  ...
  pr_mat <- re_mat <- RandInd_mat <- matrix(NA, nrow = length(tbl_algorithm), ncol = length(tbl_truth))
  ...
  for (i in 1:length(tbl_algorithm)) {
    for (j in 1:length(tbl_truth)) {
      ...
      RandInd_ij <- (true_positives + true_negatives)/(truth + negative)
      
      if (RandInd_ij == "NaN") RandInd_ij <- 0
      
      pr_mat[i, j] <- precision_ij
      re_mat[i, j] <- recall_ij
      RandInd_mat[i, j] <- RandInd_ij
    }
  }
  
  # put back cluster labels (note some row names may be missing due to removal of unassigned cells)
  rownames(pr_mat) <- rownames(re_mat) <- rownames(RandInd_mat) <- names(tbl_algorithm)
  colnames(pr_mat) <- colnames(re_mat) <- colnames(RandInd_mat) <- names(tbl_truth)
  ...
  RandInd_mat_trans <- t(RandInd_mat)
  
  if (nrow(RandInd_mat_trans) <= ncol(RandInd_mat_trans)) {
    # if fewer (or equal no.) true populations than detected clusters, can match all true populations
    labels_matched <- clue::solve_LSAP(RandInd_mat_trans, maximum = TRUE)
    # use row and column names since some labels may have been removed due to unassigned cells
    labels_matched <- as.numeric(colnames(RandInd_mat_trans)[as.numeric(labels_matched)])
    names(labels_matched) <- rownames(RandInd_mat_trans)
    ...
    labels_matched_flipped <- clue::solve_LSAP(RandInd_mat, maximum = TRUE)
    # use row and column names since some labels may have been removed due to unassigned cells
    labels_matched_flipped <- as.numeric(rownames(RandInd_mat_trans)[as.numeric(labels_matched_flipped)])
    names(labels_matched_flipped) <- rownames(RandInd_mat)
    
    labels_matched <- rep(NA, ncol(RandInd_mat))
    names(labels_matched) <- rownames(RandInd_mat_trans)
  ...
  pr <- re <- RandInd <- n_cells_matched <- rep(NA, ncol(RandInd_mat))
  names(pr) <- names(re) <- names(RandInd) <- names(n_cells_matched) <- names(labels_matched)
  
  for (i in 1:ncol(RandInd_mat)) {
    ...
    RandInd[i] <- ifelse(is.na(labels_matched[i]), 0, RandInd_mat[as.character(labels_matched[i]), names(labels_matched)[i]])
