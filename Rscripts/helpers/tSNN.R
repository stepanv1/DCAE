#find top l nearest neighbors out of k   
...
snn_rank<-relations[ , frankv(.SD$weight ,ties.method = 'random', order=-1), by="from"]$V1
l=90
relations_l<-relations[snn_rank<=l, ]

asym_w <- sparseMatrix(i=relations$from,j=relations$to,x=relations$weight, symmetric = F, index1=T);gc()
asym_w <- sparseMatrix(i=relations_l$from,j=relations_l$to,x=relations_l$weight, symmetric = F, index1=T);gc()
...
hist(colSums(asym_wn[, lbls!=0]),200)
hist(colSums(asym_wn[, ]),200)
...
helper_match_evaluate_multiple(ifelse(log(as.numeric(Ptrace$fin))< -11.8, 0,1), ifelse(lbls==0, 1, 0))
...
library(ldbod)
library(Ckmeans.1d.dp)
system.time(outl_res<-ldbod(cl_coord, k = c( 90), nsub = nrow(cl_coord),  ldf.param = c(h = 1, c = 0.1), rkof.param = c(alpha = 1, C = 1, sig2 = 1), lpdf.param = c(cov.type = "full", sigma2 = 1e-05, tmax = 1, v = 1), treetype = "kd", searchtype = "standard", eps = 0, scale.data = TRUE, method = "lof"))

hist(log10(outl_res$lof),50)
kmeansresLD<-Ckmeans.1d.dp(outl_res$lof, k=2, y=1,
                           method= "quadratic",
                           estimate.k="BIC")
plot(kmeansresLD)
#plot(Ckmeans.1d.dp(exp(logP)), k=c(1:20), y=1, estimate.k="BIC")
...
load("/mnt/f/Brinkman group/current/Stepan/RobinsonFlowComparisonALL/benchmark_data_sets/all_setsINCLUDE_NAN.RData")
lapply(data, function(x) write(x))
