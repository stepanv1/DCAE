...
cl8_center  = np.concatenate((np.zeros(4), np.ones(1), np.zeros(21), np.ones(4)), axis=0)
...
y8 = np.random.multivariate_normal(cl8_center[np.concatenate((np.zeros(4), np.ones(1), np.zeros(21), np.ones(4))).astype('bool')], r, size=ncl8)

#wd= 0.3
cl1 = cl1_center +  np.concatenate([y1, np.zeros((ncl1,original_dim - d))], axis=1 )
cl2 = cl2_center +  np.concatenate([y2, np.zeros((ncl2,original_dim - d))], axis=1 )
cl3 = cl3_center +  np.concatenate([y3, np.zeros((ncl3,original_dim - d))], axis=1 )
cl4 = cl4_center +  np.concatenate([y4, np.zeros((ncl4,original_dim - d))], axis=1 )
cl5 = cl5_center +  np.concatenate([y5, np.zeros((ncl5,original_dim - d))], axis=1 )
cl6 = cl6_center +  np.concatenate([y6, np.zeros((ncl6,original_dim - d))], axis=1 )
cl7 = cl7_center +  np.concatenate([y7, np.zeros((ncl7,original_dim - d))], axis=1 )
cl8 =  np.zeros((ncl8,original_dim ))
cl8[:,np.concatenate((np.zeros(4), np.ones(1), np.zeros(21), np.ones(4))).astype('bool')] = y8
...
cl8_noisy = cl8
cl8_noisy[:, ~np.concatenate((np.zeros(4), np.ones(1), np.zeros(21), np.ones(4))).astype('bool')] = \
...
#values = [0, 0, 1, 2, 2, 2]
colours = ListedColormap(['k','b','y','g','r', 'm','c', 'darkgrey'], N=8)
fig01 = plt.figure();
...
outfile = source_dir + '/8art_scaled_cor.npz'
...
coeffCAE = 5
epochs = 3000
ID = 'Art8'+ str(coeffCAE) + '_' + str(epochs) + '_310000samples_correlated_weighted_MMD'
...
MMD_weight_lst = K.variable(np.array(frange_anneal(epochs, ratio=0.25)))
