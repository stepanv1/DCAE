...
ID = 'ELU_graph_KL_' + '_g_'  + str(g) +  '_lam_'  + str(lam) + '_batch_' + str(batch_size) + '_alp_' + str(alp) + '_m_' + str(m)
...
for epochs in epochs_list:
    for bl in list_of_branches[0:7]:
        ...
        def graph_diff(x, y):
            #KL loss
            return g * (tf.reduce_mean(-compute_graph_weights_Inp(X) * K.log(compute_graph_weights_enc(z_mean)) + K.log(compute_graph_weights_enc(z_mean))))


        #idx = np.random.randint(0, high=nrow, size=20)
        #KL = K.eval(-compute_graph_weights_Inp(aFrame[idx,:].astype('float32')) * K.log(compute_graph_weights_enc(z[idx,:])) + K.log(compute_graph_weights_enc(z[idx,:])))
