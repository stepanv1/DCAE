'''
Runs DCAE variants. The goal is to find a way to get rif of splitting
...
for epochs in epochs_list:
    for bl in list_of_branches:
        ...
        sourceTr = aFrame
        # generate median of k neighbours
        TargetTr = np.mean(neibALL, axis=1)
        ...
        DCAEStop = EarlyStopping(monitor='DCAE_loss', min_delta=1e-5, patience=10, mode='min',
        ...
        history_multiple = autoencoder.fit([aFrame, Sigma], TargetTr,
                                           batch_size=batch_size,
                                           epochs=epochs,
                                           shuffle=True,
                                           callbacks=[AnnealingCallback(MMD_weight, MMD_weight_lst),
                                                      callSave, callPlot, DCAEStop], verbose=2)
