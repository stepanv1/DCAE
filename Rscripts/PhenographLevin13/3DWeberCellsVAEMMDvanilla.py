'''This script to visualise cytoff data using deep variational autoencoder with MMD
TODO ?? pretraining
...
autoencoder.fit(targetTr, epochs=epochs, batch_size=batch_size, shuffle=True, validation_data=(targetTr, None))
z = encoder.predict(targetTr)
...
learning_rate = 1e-3
earlyStopping = keras.callbacks.EarlyStopping(monitor='loss', min_delta=0.000000, patience=3, verbose=0, mode='min',
                                              restore_best_weights=True)

#adam = Adam(lr=learning_rate, epsilon=0.001)

# ae.compile(optimizer=adam, loss=ae_loss)
vae.compile(optimizer='adam',  loss=vae_loss(kl_weight, kl_weight_lst), metrics=[kl_loss, mean_square_error_NN, contractive])
# ae.get_weights()
...
history = vae.fit([sourceTr, neibF_Tr,  Sigma, weight_neibF], targetTr,
                  batch_size=batch_size,
                  epochs=epochs,
                  shuffle=True,
                  callbacks=[CustomMetrics(), tensorboard, AnnealingCallback(kl_weight, kl_weight_lst)])
...
# analog of tsne plot fig15 from Nowizka 2015, also see fig21
#for m in range(len(markers[0:12])):
m=4
plot([Scatter3d(x=x, y=y, z=zz,
                    mode='markers',
                    marker=dict(
