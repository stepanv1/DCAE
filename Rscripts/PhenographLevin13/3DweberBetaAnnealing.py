...
perp(np.ascontiguousarray(weight_distALL), nrow, original_dim, np.ascontiguousarray(weight_neibALL), k, k * 3, Sigma,
     12)
...
def mean_square_error_NN(y_true, y_pred):
    ...
    weightedN = k * original_dim * K.dot(dst,
                                         K.transpose(weight_neib))  # not really a mean square error after wwe done this
...
vae.compile(optimizer='adam',  loss=vae_loss(kl_weight, kl_weight_lst), metrics=[kl_loss, mean_square_error_NN])
# ae.get_weights()
