...
#Set the variables
    ...
    tree = KDTree(data, metric='l2')
    dist, ind = tree.query(data, k=16, dualtree = True)
    ind += 1
    ...
    jaccard30 = parallel_jaccard_kernel(ind[:,1:16])

    j30list = [list(y) for y in [zip([z]*15, jaccard30[z][0], jaccard30[z][1]) for z in [x for x in range(len(jaccard30))]]]
    ...
    np.savetxt(ROOT + '/results/'+ CALC_NAME+"/"+"j15" + SET_NAME + ".txt", newline='\n', fmt=['%d', '%d', '%f'], X = j30st)