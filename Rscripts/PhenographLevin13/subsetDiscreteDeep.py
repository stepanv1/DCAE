...
This script is to balance sample size of eah subset from gated data and to see what happnes
...
h2 = Dense(intermediate_dim2, activation='relu')(x)
#h2 = BatchNormalization()(h2)
h = Dense(intermediate_dim, activation='relu')(h2)
...
def custom_loss(x, x_decoded_mean, z_mean):
    ...
    #print(g_l)
    return 1*msew + loss_mmd + contractive() - 1*g_l + 0*entropy(x, x_decoded_mean)
...
epochs = 2000
min_temperature= 0.5
anneal_rate = abs(np.log( min_temperature/max_temperature )/(epochs-5))
for e in range(epochs):
    print('epoch', e)
    history = autoencoder.fit([targetTr[1000:172600,:], neibF_Tr[1000:172600,:],  Sigma[1000:172600], weight_neibF[1000:172600,:]],
                epochs=1, batch_size=batch_size,
                validation_data=([targetTr[0:1000,:], neibF_Tr[0:1000,:],  Sigma[0:1000], weight_neibF[0:1000,:]], None), #shuffle=True,
...
plot([Scatter(x=x, y=y,
                ...
                hoverinfo='text')], filename='2dPerplexityk30weighted200epochs_Patient_Treatment_LAM_0.001.html')
