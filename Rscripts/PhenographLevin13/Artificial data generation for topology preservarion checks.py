...
cl3_center = np.concatenate((2*np.ones(1), np.zeros(original_dim-1)), axis=0 )
cl4_center = np.concatenate((3*np.ones(1), np.zeros(original_dim-1)), axis=0 )
cl5_center = np.concatenate((4*np.ones(1), np.zeros(original_dim-1)), axis=0 )
cl6_center = np.concatenate((3*np.ones(1), np.zeros(original_dim-1)), axis=0 )
cl6_center[1] = 1
cl7_center = np.concatenate((4*np.ones(1), np.zeros(original_dim-1)), axis=0 )
cl7_center[2] = 1
# cluster populatiosn
ncl1 = ncl2 = ncl3 = ncl4  = ncl5 = ncl6 = ncl7 = 30000
# cluster labels
...
plt.scatter(y[:, 0], y[:, 1], c=lbls, s=1., cmap=plt.cm.Spectral)
...
coeffCAE = 5
epochs = 2000
ID = 'Art7'+ str(coeffCAE) + '_' + str(epochs) + '_210000samples'
