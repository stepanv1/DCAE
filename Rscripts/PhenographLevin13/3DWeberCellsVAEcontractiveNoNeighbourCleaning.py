...
def contractive(x, x_decoded_mean):
        W = K.variable(value=vae.get_layer('z_mean').get_weights()[0])  # N x N_hidden
        W = K.transpose(W)  # N_hidden x N
        h = vae.get_layer('z_mean').output
...
def vae_loss(weight, kl_weight_lst):
    def loss(x, x_decoded_mean):
        ...
        return K.mean(msew + kl_weight * kl_loss(x, x_decoded_mean) + 1*contractive(x, x_decoded_mean))
        # return K.mean(msew)
    return loss


# y = CustomVariationalLayer()([x, x_decoded_mean])
vae = Model([x, neib, SigmaTsq, weight_neib], x_decoded_mean)
vae.summary()
