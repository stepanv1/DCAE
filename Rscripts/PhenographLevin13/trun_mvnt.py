...
def rtmvn(n=1, Mean=np.zeros(1), Sigma=np.ones(1), \
          ...
          burn=10, thin=1, ini=[]):
    D = np.matrix(D);
    [m, p] = D.shape  # m constraints p-dimensional TN
    ...
    if Mean.size != p or lower.size != m or upper.size != m:  # check Mean an D matrix dimension
        print(
            'dimension of "Mean should match with No. col of "D". Dimension of "lower" and "upper" should match with No. row of "D"!')

        return []
    else:
        Mean = Mean.reshape((p, 1))
        lower = lower.reshape((m, 1));
        upper = upper.reshape((m, 1))
