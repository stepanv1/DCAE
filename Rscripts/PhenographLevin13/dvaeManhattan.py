    ...
    def on_epoch_end(self, epoch, logs=None):
        for k in logs:
            ...
            if k.endswith('kl_l'):
                print
                logs[k]
            if k.endswith('categorical_accuracy'):
                print
...
#cutoff=np.repeat(np.mean(cutoff), 38)
#model parameters
batch_size = 10
original_dim = 38
...
N=38 # number of categorical distributions
latent_dim = M*N
...
def singleInput(i):
     ...
     cut_nei = np.array([1 for j in range(original_dim)])
#weighted distances computed in L1 metric
     weight_di = [sum((np.square(features[i] - nei[k_i,]) )) for k_i in rk]
...
# Create AE and training
    ...
    ae.fit(X_train_tmp, X_train_tmp, batch_size=10, epochs=20)
...
encoded1 = Dense(intermediate_dim, activation='relu')(x)
...
tau = K.variable(50.0, name="temperature")
...
x_decoded3 = Dense(intermediate_dim, activation='relu')(y)

x_decoded4 = Dense(original_dim, activation='relu')(x_decoded3)
vae = Model([x, neib, cut_neib, weight_neib], x_decoded4)
...
def vae_loss(x, x_decoded_mean):
    ...
    msewNN = original_dim * mean_square_error_NN(x, x_decoded_mean)
...
vae.compile(optimizer=adam, loss=vae_loss, metrics=[mean_square_error_weighted,  mean_square_error_NN, gumbel_loss])
#ae.get_weights()
...
for e in range(epochs):
    print('epoch', e)
    history=vae.fit([sourceTr, neibF_Tr, cut_neibF_Tr, weight_neibF_Tr], targetTr,
    batch_size=batch_size,
    epochs = 1,
    shuffle=True,
    callbacks=[CustomMetrics(),  tensorboard])
    K.set_value(tau, np.max([K.get_value(tau) * np.exp(np.log(min_temperature/max_temperature)/epochs), min_temperature]))
    print('temperature', K.get_value(tau))
stop = timeit.default_timer()
ae.save('Wang0_modelMnhattanDVAE.h5')
...
x_test_vae = vae.predict([sourceTr, neibF_Tr, cut_neibF_Tr, weight_neibF_Tr])
len(x_test_vae)
np.savetxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_dvae.txt', x_test_vae)
...
argmax_y = K.reshape(logits_y,(-1, N, M))
encoder = K.function([x], [argmax_y, x])
code, x_hat_test = encoder([aFrame])
...
counts
counts.shape
unique, counts = np.unique(codeFlat[lbls==7,], axis=0, return_counts=True)
print(counts)
counts.shape
...
kmo = KMeans(n_clusters=15,  n_init=10)
...
data3D=np.loadtxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_enc3d.txt')
...
plot([Scatter3d(x=x,y=y,z=z,
        ...
        marker=dict(
        size=0.5,
        color = kml/100,                # set color to an array/list of desired values
...
plot([Scatter3d(x=x,y=y,z=z,
                mode='markers',
        marker=dict(
        size=[2 if x==cl else 0.5 for x in kml],
        color = ['red' if x==cl else 'green' for x in kml],                # set color to an array/list of desired values
        colorscale='Viridis',   # choose a colorscale
        opacity=0.5,
            ),
                text=popnames,
...
kmlANDpopnames= np.core.defchararray.add(np.core.defchararray.add(kml.astype(str), '  ') , popnames)

plot([Scatter3d(x=x,y=y,z=z,
                mode='markers',
        marker=dict(
        size=0.5,
        color = kml,                # set color to an array/list of desired values
        colorscale='Viridis',   # choose a colorscale
        opacity=0.9,
            ),
                text=kmlANDpopnames,
...
import fastcluster
ljaccard=fastcluster.linkage_vector(codeFlat, method='single', metric='jaccard')
t0 = time()
print('jaccard', time() - t0)

lsingle=fastcluster.linkage_vector(codeFlat, method='single', metric='cityblock')
t0 = time()
print('single', time() - t0)
lmedian=fastcluster.linkage_vector(codeFlat, method='ward', metric='euclidean')
t0 = time()
print('median', time() - t0)

laverage=fastcluster.linkage(codeFlat, method='average', metric='cityblock')
t0 = time()
print('average', time() - t0)



from scipy.cluster.hierarchy import cut_tree

treejaccard=cut_tree(ljaccard, n_clusters=[3,15,30], height=None)
treesingle=cut_tree(lsingle, n_clusters=[3,15,30], height=None)
treemedian=cut_tree(lmedian, n_clusters=[3,15,30], height=None)
treeaverage=cut_tree(laverage, n_clusters=[3,15,30], height=None)
...
index.createIndex({'post': 2,'efConstruction':'1000','M':64}, print_progress=True)

neighbours = index.knnQueryBatch(data, k=100, num_threads=40)
err=[i for i in  range(len(neighbours)) if neighbours[i][0][0]!=i]
nn_err=[np.array(neighbours[i][0]) for i in  range(len(neighbours)) if neighbours[i][0][0]!=i]
errMAt=np.stack(nn_err, axis=0)

dataERR=np.stack([data[i,] for i in  range(len(neighbours)) if neighbours[i][0][0]!=i], axis=0)
neighboursNEW = index.knnQueryBatch(dataERR, k=100, num_threads=40)


from sklearn.neighbors import KDTree
kdt = KDTree(data, leaf_size=30, metric='euclidean')
correct=kdt.query(data[err], k=100, return_distance=True)
...
#generate blobs to create same size of data in previous
from sklearn.datasets.samples_generator import make_blobs
data, y = make_blobs(n_samples=100000, centers=20, n_features=100, random_state=1)
data = np.random.randn(100000, 100).astype(np.float32)
# initialize a new index, using a HNSW index on Cosine Similarity
index = nmslib.init(method='hnsw', space='l2')
index.addDataPointBatch(data)
...
neighbours = index.knnQueryBatch(data, k=10, num_threads=40)
err=[neighbours[i][0][0]!=i for i in  range(len(neighbours))]
sum(err)
err2=[len(neighbours[i][0])!=10 for i in  range(len(neighbours))]
sum(err2)
