...
def custom_loss(x, train_xr, train_z):
    ...
    print(embedding.fit_transform(x))
    #add mds loss
    x_scaled = embedding.fit_transform(x)
    print('x_scaled', K.eval(x_scaled))
    #loss_mds = keras.losses.mean_squared_error(train_z, x_scaled)
    return msew + loss_mmd + 1*contractive() #+ 0.1 * loss_mds
...
history = autoencoder.fit([targetTr[0:172600,:], neibF_Tr[0:172600,:,:],  Sigma[0:172600], weight_neibF[0:172600,:]],
                epochs=epochs, batch_size=batch_size)#, sh
...
#sourceTr, source_test, neibFTr, neibF_test,  SigmaTr, Sigma_test, weight_neibFTr, weight_neibF_test= \
#    train_test_split(aFrame, neibF,  Sigma, weight_neibF, test_size=0.15, shuffle= True)

#history = autoencoder.fit([targetTr[0:146800,:], neibFTr[0:146800,:,:],  SigmaTr[0:146800], weight_neibFTr[0:146800,:]],
#                          epochs=epochs, batch_size=batch_size, shuffle=True,
#                          validation_data=(([source_test[0:25800,:], neibF_test[0:25800,:,:], Sigma_test[0:25800], weight_neibF_test[0:25800,:]], None)))
z = encoder.predict([sourceTr, neibF_Tr,  Sigma, weight_neibF])
