...
data0 = np.genfromtxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/rln4.txt'
...
, names=None, dtype=int, skip_header=0)
...
communities, graph, Q = cluster(aFrame[idxk], k=60, nn_method='vptree', n_jobs=12)
communitiesC, graphC, QC = cluster(x_test_vae[idxk], k=60, nn_method='vptree', n_jobs=12)
table(communities);
table(communitiesC);
Q
QC
idxk=lbls!=11, lbls!=1, lbls!=6, lbls!=14
idxk=lbls!=14, lbls!=25
print(sklearn.metrics.adjusted_mutual_info_score(communities,
                                                             lbls[np.logical_and.reduce((idxk))]))

print(sklearn.metrics.adjusted_mutual_info_score(communitiesC,
                                                             lbls[np.logical_and.reduce((idxk))]))
...
import phenograph
#pt3=patient[np.logical_or.reduce((patient==4, patient==5, patient==6))]
communities, graph, Q = phenograph.cluster(aFrame[idxk], k=60, nn_method='kdtree', n_jobs=12, primary_metric='manhattan')
communitiesC, graphC, QC = phenograph.cluster(x_test_vae[idxk], k=60, nn_method='kdtree', n_jobs=12,  primary_metric='manhattan')
table(communities);
table(communitiesC);
Q
QC
