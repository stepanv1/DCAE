...
for iii in range(10):
    ...
    epochs = epochs,
    shuffle=True,
    callbacks=[CustomMetrics(), logger,  GetBest(monitor='val_loss', verbose=1, mode='min'),earlyStopping])
    stop = timeit.default_timer()
...
connClean2 = [sum((stats.itemfreq(lbls[nb2['idx'][x,:]])[:,1] / k)**2) for x in range(len(aFrame)) ]
plt.figure();plt.hist(connClean2,50);plt.show()
nb3=find_neighbors(x_test_enc2, k, metric='L2', cores=12)
...
weight_dist_enc = np.zeros((nrow, k3))
weight_neib_enc = np.zeros((nrow, k3))
k=60
neibMat=find_neighbors(x_test_enc2, k3, metric='L2', cores=12)
weight_dist_enc=neibMat['dist']
perp(np.sqrt(weight_dist_enc[:,0:k3]),   nrow,     intermediate_dim,   weight_neib_enc,          k,          k*3,        12)
#(               double* dist,           int N,            int D,          double* P,     double perplexity, int K, int num_threads)
...
adj_mat =1/2*(adj_mat+adj_mat.transpose(copy=True))
import igraph
