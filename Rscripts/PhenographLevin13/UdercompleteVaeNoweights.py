...
lib = ctypes.cdll.LoadLibrary("/mnt/f/Brinkman group/current/Stepan/Clibs/perp2.so")
perp = lib.Perplexity
perp.restype = None
...
perp((weight_distALL[:,0:k3]),   nrow,     38,   weight_neibALL,          k,         k*3,        sigmaTsne,         12)
...
def mean_square_error_NN(y_true, y_pred):
    ...
    return 0.5* K.sum(tf.multiply(dst, 1/SigmaTsq))

def kl_loss(x, x_decoded_mean):
    return - 0.5 * k * K.sum(1 + z_log_var - K.square(z_mean) - K.exp(z_log_var), axis=-1)
...
vae = Model([x, neib, cut_neib, SigmaTsq], x_decoded_mean)
vae.summary()
...
history=vae.fit([sourceTr, neibF_Tr, cut_neibF_Tr, sigmaTsne], targetTr,
...
encoder = Model([x, neib, cut_neib, SigmaTsq], z_mean)
...
x_test_vae = vae.predict([sourceTr, neibF_Tr, cut_neibF_Tr, sigmaTsne])
len(x_test_vae)
...
x_test_enc = encoder.predict([sourceTr, neibF_Tr, cut_neibF_Tr, sigmaTsne])
