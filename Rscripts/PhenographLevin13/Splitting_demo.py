...
k = (y1 - 0.0)/(x1 - 0.43)
b = y1 - k * x1
d_set_inp_x = np.linspace(0.43,0.9696, nx)
...
np.savez(output_dir + '/' + ID11 + "_" +  'epochs' + str(epochs) + '_aFrame.npz', aFrame=aFrame)
npzfile = np.load(output_dir + '/' +ID11 + "_" +  'epochs' + str(epochs) + '_aFrame.npz')
aFrame = npzfile['aFrame']
...
history_multiple = autoencoder.fit(aFrame, aFrame,
                                   ...
                                   epochs=epochs,
                                   shuffle=True,
                                   callbacks=[saveEncoder(encoder=encoder, ID=ID11, epochs=epochs,
                                                                  output_dir=output_dir, save_period=save_period)],
                                   verbose=1)
stop = timeit.default_timer()
z = encoder.predict([aFrame])
...
A_rest = autoencoder.predict(aFrame)
z = encoder.predict(aFrame)
print(output_dir + '/autoencoder_' +ID11  + "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')
...
np.savez(output_dir + '/' + ID12 + "_" +  'epochs' + str(epochs) + '_aFrame.npz', aFrame=aFrame)
npzfile = np.load(output_dir + '/' +ID12 + "_" +  'epochs' + str(epochs) + '_aFrame.npz')
aFrame = npzfile['aFrame']
...
history_multiple = autoencoder.fit(aFrame, aFrame,
                                   ...
                                   epochs=epochs,
                                   shuffle=True,
                                   callbacks=[saveEncoder(encoder=encoder, ID=ID12, epochs=epochs,
                                                                  output_dir=output_dir, save_period=save_period)],
                                   verbose=1)
stop = timeit.default_timer()
z = encoder.predict([aFrame])
...
A_rest = autoencoder.predict(aFrame)
z = encoder.predict(aFrame)
print(output_dir + '/autoencoder_' +ID12  + "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')
...
################################################################################################
ID14 = 'step_DELU_encoder_cont_decoder_discont'
# https://github.com/jloveric/high-order-layers
...
np.savez(output_dir + '/' + ID14 + "_" +  'epochs' + str(epochs) + '_aFrame.npz', aFrame=aFrame)
npzfile = np.load(output_dir + '/' +ID14 + "_" +  'epochs' + str(epochs) + '_aFrame.npz')
aFrame = npzfile['aFrame']
...
h2= Dense(intermediate_dim2, activation=DELU_activation, name='intermediate2', kernel_initializer=initializer)(h)
z_mean = Dense(latent_dim, activation='tanh', name='z_mean', kernel_initializer=initializer)(h2)
...
def DCAE_3l(y_true, y_pred):  # (x, x_decoded_mean):  # attempt to avoid vanishing derivative of sigmoid
    U = encoder.get_layer('intermediate').trainable_weights[0]
    W = encoder.get_layer('intermediate2').trainable_weights[0]
    Z = encoder.get_layer('z_mean').trainable_weights[0]  # N x N_hidden
    U = K.transpose(U);
...
history_multiple = autoencoder.fit(aFrame, aFrame,
                                   ...
                                   epochs=epochs,
                                   shuffle=True,
                                   callbacks=[saveEncoder(encoder=encoder, ID=ID14, epochs=epochs,
                                                                  output_dir=output_dir, save_period=save_period)],
                                   verbose=1)
stop = timeit.default_timer()
z = encoder.predict([aFrame])
print(stop - start)
#
encoder.save_weights(output_dir + '/' +ID14 + "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')
autoencoder.save_weights(output_dir + '/autoencoder_' +ID14 + "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')

np.savez(output_dir + '/' +ID14+ "_linear_" +  'epochs' + str(epochs) + '_latent_rep_3D.npz', z=z)
with open(output_dir + '/' +ID14 + "_linear"+'epochs' + str(epochs) + '_history', 'wb') as file_pi:
  pickle.dump(history_multiple.history, file_pi)

encoder.load_weights(output_dir + '/' +ID14 +  "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')
autoencoder.load_weights(output_dir + '/autoencoder_' +ID14  + "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')
A_rest = autoencoder.predict(aFrame)
z = encoder.predict(aFrame)
print(output_dir + '/autoencoder_' +ID14  + "_linear_"  + 'epochs' + str(epochs) + '_3D.h5')

# extract decoder
decoder_input = Input(shape=(latent_dim,))
x= Dense(intermediate_dim2, activation=DELU_activation, name='intermediate3', kernel_initializer=initializer)(decoder_input )
decoder_h2 = Dense(intermediate_dim, activation=DELU_activation, name='intermediate4', kernel_initializer=initializer)(x)
decoded= Dense(original_dim, activation=DELU_activation, name='output', kernel_initializer=initializer)(decoder_h2)
decoder = Model(inputs=decoder_input, outputs=decoded)
decoder.summary()
weights_list = autoencoder.get_weights()[4:8]# change this
decoder.set_weights(weights_list)
dec_map = decoder.predict(np.linspace(-1, 1, num=1000))
...
history = pickle.load(open(output_dir + '/' +ID14 + "_linear"+'epochs' + str(epochs) + '_history',  "rb"))
st = 1000;
stp = 100000 #len(history['loss'])
fig01 = plt.figure();
plt.plot((history['loss'][st:stp]));
plt.title('loss')
fig = plt.figure();
plt.plot((history['DCAE_3l'][st:stp]));
plt.title('DCAE_3l')
