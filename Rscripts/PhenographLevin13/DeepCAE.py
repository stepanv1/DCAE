...
def custom_loss(x, x_decoded_mean, z_mean):
    ...
    #
    return 1*msew + 2*loss_mmd(x, x_decoded_mean) + 0.75*deep_contractive(x, x_decoded_mean)
################################################################################################
#################################################################################################
...
plot([Scatter(x=x, y=y,
                ...
                hoverinfo='text')], filename='New_LOSS_500epochs_LAM_0.001_MMD_2_DCAE_0.75_scaled.html')
...
# pZap70 inNK cells :https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2199178/;
#marker_sub=markers
sub_idx =  np.random.choice(range(len(lbls)), 10000, replace  =False)
x = z[sub_idx, 0]
y = z[sub_idx, 1]
...
fig.show()
plotly.offline.plot(fig, filename='DCAE_deep_reluMMD2_DCAE_0.75allMarkers.html')