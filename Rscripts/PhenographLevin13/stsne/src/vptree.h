...
#pragma omp for
  for (int n = 0; n < num_rows; n++) {
    tree->search_s(&data[n * dim], dim, num_neighbors + 1, &indices_matrix[num_neighbors * n], &distance_matrix[num_neighbors * n]);
  }
}
  }
...
#pragma omp parallel for
    for (int n = 0; n < num_rows; n++) {
      ...
      double * local_distances;
      local_distances = new double[num_neighbors + 1];
      //tree->search(*data + (n * dim), num_neighbors + 1, local_indices, local_distances);
      for (int k = 0; k < num_neighbors; k++) {
        indices_matrix[row[n] + k] = local_indices[k + 1].index();
        distance_matrix[row[n] + k] = local_distances[k + 1];
      }
