...
#np.savetxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_enc333d.txt', x_test_enc)
#x_test_enc=np.loadtxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_enc3d.txt')
...
core = popnames in ['CD4_CM', 'CD4_EM', 'CD4_EMRA', 'CD4_Naive',  'CD8_CM', 'CD8_EM',  'CD8_EMRA', 'CD8_Naive', 'NK',  'NKT ']
coreNames=['CD4_CM', 'CD4_EM', 'CD4_EMRA', 'CD4_Naive',  'CD8_CM', 'CD8_EM',  'CD8_EMRA', 'CD8_Naive', 'NK',  'NKT ']
...
plot([Scatter3d(x=x,y=y,z=z,
        ...
        marker=dict(
        size=1,
        color = [lbls[i]/5 if (popnames[i]  in coreNames) else 'grey' for i in range(x_test_enc.shape[0])],#
...
plot([Scatter3d(x=x,y=y,z=z,
        ...
        marker=dict(
        size=0.5,
        color = lbls/100,                # set color to an array/list of desired values
        colorscale='Viridis',   # choose a colorscale
        opacity=0.9,
            ),
                text=popnames,
...
plot([Scatter(x=x,y=y,
        ...
        marker=dict(
        size=2,
        color = [lbls[i]/5 if (popnames[i]  in coreNames) else 'grey' for i in range(x_test_enc.shape[0])],# set color to an array/list of desired values
        colorscale='Rainbow',   # choose a colorscale
        opacity=0.5,
            ),
                text=popnames,
...
plot([Scatter(x=x,y=y,
        ...
        marker=dict(
        size=2,
        color = lblsCore/5,                # set color to an array/list of desired values
        colorscale='Viridis',   # choose a colorscale
        opacity=1,
            ),
                text=popnames,
...
#np.savez('otsnePertnewlbls.npzMulti', Y0=Y0)
zzz=np.load('/home/sgrinek/anaconda3/otsnePertnewlbls.npzMulti.npz')
Y0=zzz['Y0']
import random
...
for g in np.unique(lbls):
    i = np.where(lbls== g)
    ax.scatter(Y0[i,0], Y0[i,1], label=g, marker='.', s=1, c=color[g] if (popnames[i[0][0]]  in coreNames) else 'grey' )
ax.legend(markerscale=10)

plot([Scatter(x=Y0[:,0],y=Y0[:,1],
                mode='markers',
        marker=dict(
        size=2,
        color = [lbls[i]/5 if (popnames[i]  in coreNames) else 'grey' for i in range(x_test_enc.shape[0])],# set color to an array/list of desired values
...
Y03D = tsne.fit_transform(aFrame)
np.savez('otsnePertMAnhattanDenoise.npz', Y03D=Y03D)
...
plot([Scatter3d(x=Y03D[:,0],y=Y03D[:,1], z=Y03D[:,2],
                mode='markers',
        marker=dict(
        size=2,
        color = [lbls[i]/5 if (popnames[i]  in coreNames) else 'grey' for i in range(x_test_enc.shape[0])],# set color to an array/list of desired values
