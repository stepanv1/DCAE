...
epsilon_std = 1.0
U=10#energy barier
r=5#number of replicas in training data set
...
np.savez('otsnel1.npz', Y0o=Y0o, Y1o=Y1o)
...
fig, ax = plt.subplots()
for g in np.unique(lbls[lbls!=11]):
    i = np.where(lbls[lbls!=11] == g)
...
communities, graph, Q = cluster(aFrame, k=30, nn_method='vptree', n_jobs=12)
communitiesC, graphC, QC = cluster(x_test_vae, k=30, nn_method='vptree', n_jobs=12)
table(communities);
table(communitiesC);
Q
QC
print(sklearn.metrics.adjusted_mutual_info_score(communities[np.logical_and.reduce((lbls!=11, lbls!=15, lbls!=9, lbls!=4))],
                                                             lbls[np.logical_and.reduce((lbls!=11, lbls!=15, lbls!=9, lbls!=4))]))

print(sklearn.metrics.adjusted_mutual_info_score(communitiesC[np.logical_and.reduce((lbls!=11, lbls!=15, lbls!=9, lbls!=4))],
                                                             lbls[np.logical_and.reduce((lbls!=11, lbls!=15, lbls!=9, lbls!=4))]))
