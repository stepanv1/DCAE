...
IDX=np.logical_or.reduce((patient==4, patient==5, patient==6))
...
# Create AE and training
    ...
    encoder = Dense(n_out, activation='tanh', W_regularizer=l2(l2_penalty_pre))(pretrain_input)
    decoder = Dense(n_in, activation='linear', W_regularizer=l2(l2_penalty_pre))(encoder)
...
encoded1 = Dense(intermediate_dim, activation='selu',W_regularizer=l1(l1_penalty_ae))(x)

encoded2 = Dense(latent_dim, activation='selu', W_regularizer=l1(l1_penalty_ae))(encoded1)

x_decoded3 = Dense(intermediate_dim, activation='selu', W_regularizer=l1(l1_penalty_ae))(encoded2)

x_decoded4 = Dense(original_dim, activation='relu',W_regularizer=l1(l1_penalty_ae))(x_decoded3)
...
#x_test_vae=np.loadtxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_ae001.txt')
...
ae.set_weights(trained_weight)
del weight_neibALL, weight_distALL
#ae.get_weights()#ae.get_weights()
...
from sklearn.manifold import TSNE as oldTSNEcolors = ['red', 'brown', 'yellow', 'green', 'blue']
...
#np.savez('otsne.npz', Y0o=Y0o, Y1o=Y1o, Y0=Y0, Y1=Y1)
...
fig, ax = plt.subplots()
for g in np.unique(patient[(lbls!=11)]):
...
tsne = TSNE(n_jobs=12,  n_iter=5000)
Y1all = tsne.fit_transform(x_test_vae)importlib.reload(phenograph2)
...
m=35
fig, ax = plt.subplots()
im=ax.scatter(Y0all[:,0], Y0all[:,1],  marker='.', s=0.2, c=data0[:,m], cmap=cmap, alpha=0.9, )
...
communities, graph, Q = cluster(aFrame[pt3==pt,:], k=30, nn_method='vptree', n_jobs=12)
communitiesC, graphC, QC = cluster(x_test_vae[pt3==pt,:], k=30, nn_method='vptree', n_jobs=12)
table(communities);
table(communitiesC);
print(sklearn.metrics.adjusted_mutual_info_score(communities[lbls!=11], lbls[np.logical_and(patient==pt , lbls!=11)]))
print(sklearn.metrics.adjusted_mutual_info_score(communitiesC[lbls[patient==pt]!=11], lbls[np.logical_and(patient==pt , lbls!=11)]))


pt=4
import phenograph
pt3=patient[np.logical_or.reduce((patient==4, patient==5, patient==6))]
communities, graph, Q = phenograph.cluster(aFrame[pt3==pt,:], k=100, nn_method='kdtree', n_jobs=12)
communitiesC, graphC, QC = phenograph.cluster(x_test_vae[pt3==pt,:], k=100, nn_method='kdtree', n_jobs=12)
table(communities);
table(communitiesC);
