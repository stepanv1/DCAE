'''This script to visualise cytoff data using deep variational autoencoder with
...
import glob
from keras.layers import Input, Dense, Lambda, Concatenate, Layer, Dropout, BatchNormalization
...
IDX = np.hstack([ i[:1500] for i in [np.arange(nrow)[lbls == j] for j in np.unique(lbls)] ])
lbls = lbls[IDX]
aFrame = aFrame[IDX,:]
...
#  p = Dense(1, bias=0, W_regularizer=regularizers.l1(0.01),W_constraint=nonneg())(input_sequences)
class TestConstraint(Constraint):
    def __init__(self, N, target_sum=1):
...
cat_prob=Dense(M*N, use_bias=False,  name='cat_prob', kernel_initializer=keras.initializers.Constant(value=np.log(1/(M*N)) ))(logits_y)

def sampling(logits_y):
    U = K.random_uniform(K.shape(logits_y), 0, 1)
    ly = logits_y - K.log(-K.log(U + 1e-20) + 1e-20) # logits + gumbel noise
...
# concatenate y and z_mean

merged  =  Concatenate()([z_mean, y])
encoder = Model([x, neib, SigmaTsq, weight_neib], z_mean, name='encoder')
encoderProbs = Model([x, neib, SigmaTsq, weight_neib], cat_prob, name='encoderProbs')
encoderDiscrete = Model([x, neib, SigmaTsq, weight_neib], logits_y, name='encoderDiscrete')
encoderDiscreteSample = Model([x, neib, SigmaTsq, weight_neib], y, name='encoderSample')
# we instantiate these layers separately so as to reuse them later
#decoder_input = Input(shape=(latent_dim,))

decoder_h = Dense(intermediate_dim, activation='relu')
decoder_mean = Dense(original_dim, activation='relu')
h_decoded = decoder_h(merged)
...
def gumbel_loss(x, x_hat):
    # prior weights
    pw = K.variable(value=encoderProbs.get_layer('cat_prob').get_weights()[0])# Here weights are
    #
    q_y = K.reshape(logits_y, (-1, N, M))
    q_y = softmax(q_y)
    log_q_y = K.log(q_y + 1e-20)
    kl_tmp = q_y * (log_q_y -  K.reshape(cat_prob,(-1, N, M))  + K.sum(K.exp(pw), axis=0) )# FIgure out Axis
...
def custom_loss(x, x_decoded_mean, z_mean):
    ...
    #print(g_l)
    return 1*msew + loss_mmd + contractive() - g_l + entropy(x, x_decoded_mean)

loss = custom_loss(x, x_decoded_mean, z_mean)
autoencoder.add_loss(loss)
autoencoder.compile(optimizer='adam', metrics=[mean_square_error_NN, contractive,  custom_loss, gumbel_loss])
...
min_temperature= 0.5
anneal_rate = abs(np.log( min_temperature/max_temperature )/(epochs-1))
for e in range(epochs):
    print('epoch', e)
    history = autoencoder.fit([targetTr[600:172600,:], neibF_Tr[600:172600,:],  Sigma[600:172600], weight_neibF[600:172600,:]],
                epochs=1, batch_size=batch_size,
                validation_data=([targetTr[0:600,:], neibF_Tr[0:600,:],  Sigma[0:600], weight_neibF[0:600,:]], None), #shuffle=True,
                    callbacks=[CustomMetrics()])#, validation_data=([targetTr, neibF_Tr,  Sigma, weight_neibF], None))
    K.set_value(tau, np.max([K.get_value(tau) * np.exp(- anneal_rate), min_temperature]))
    print('temperature', K.get_value(tau))

#[targetTr[2000:88400,:], neibF_Tr[2000:88400,:],  Sigma[2000:88400], weight_neibF[2000:88400,:]]



z = encoder.predict([aFrame, neibF,  Sigma, weight_neibF])
g_z = encoderDiscrete.predict([aFrame, neibF,  Sigma, weight_neibF])
g_zSample = encoderDiscreteSample.predict([aFrame, neibF,  Sigma, weight_neibF])
clus = np.argmax(g_z, axis=1)
clusSample = np.argmax(g_zSample, axis=1)
table(clus)
table(clusSample)
#encoder.save_weights('encoder_weightsWEBERCELLS_2D_MMD_CONTRACTIVEk30_2000epochs_LAM_0.001.h5')
import sklearn
sklearn.metrics.f1_score(lbls, clus, average='weighted')
sklearn.metrics.f1_score(lbls, clusSample, average='weighted')
# checking clustering results
from sklearn.metrics import confusion_matrix
confusion_matrix(lbls, clus)





#encoder.load_weights('encoder_weightsWEBERCELLS_2D_MMD_CONTRACTIVEk30_200epochs_LAM_0.00001.h5')
...
plot([Scatter(x=x, y=y,
                ...
                text=clust,
                hoverinfo='text')], filename='Discrete2dPerplexityk30weighted200epochs_LAM_0.00001.html')
...
plot([Scatter(x=x, y=y,
                ...
                marker=dict(
                    size=2,
                    color=clus,  # set color to an array/list of desired values
                    colorscale='Viridis',  # choose a colorscale
                    opacity=0.5,
                ),
                text=clust,
                hoverinfo='text')], filename='DiscreteClustering2dPerplexityk30CLUS.html')
...
#m=10
    plot([Scatter(x=x, y=y,
                        ...
                        text=clust,
                        hoverinfo='text'
                        )], filename='Balanced2dPerplexityk30weighted100epochs_LAM_0.00001_' + markers[m] + '.html')
...
plot([Scatter(x=x, y=y,
                ...
                hoverinfo='text')], filename='Entropy_added_1700epochs_Patient_Treatment.html')
...
plot([Scatter(x=x, y=y,
                ...
                text=patient_table[:,1],
                hoverinfo='text')], filename='Entropyreg1700epochs_Treatment_LAM_0.001.html')
