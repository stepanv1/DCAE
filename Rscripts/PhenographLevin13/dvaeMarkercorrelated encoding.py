...
def find_neighbors(data, k_, metric='euclidean', cores=12):
    ...
    #correct results shifting some to the right
    IDX = distances[:,0]!=0
...
#calculate pre-codes for regularisation
meanW=  np.transpose(np.array([np.einsum('...i,...i',weight_neibF[:,:] , neibF[:,:,i]) for i in range(original_dim)], order='F' ))
bins = np.array([0.0, 0.1, 0.5,1])
digs = np.digitize(meanW, bins)-1
n_values = np.max(digs) + 1
digs1hot=(np.eye(n_values)[digs]).reshape((nrow, original_dim*3))
...
preenc = Input(shape = (digs1hot.shape[1], ))
...
ae = Model([x, neib, cut_neib, weight_neib, preenc], x_decoded4)
...
def mean_square_error_weightedNN(y_true, y_pred):
    #dst = original_dim *K.mean(K.square((neib  - K.expand_dims(y_pred, 1)) ), axis=-1)
    dst = original_dim *K.mean(K.square((neib - K.expand_dims(y_pred, 1)) / (tf.expand_dims(cut_neib,1) + 1) ), axis=-1)
    ww = weight_neib
...
def gumbel_loss(x, x_hat):
    ...
    elbo =  bce(preenc, logits_y) - 0.001*KL
    #elbo = - KL
    return elbo
...
for e in range(epochs):
    print('epoch', e)
    history=ae.fit([sourceTr, neibF_Tr, cut_neibF_Tr, weight_neibF_Tr, digs1hot], targetTr,
    batch_size=batch_size,
    epochs = 1,
    shuffle=True,
    callbacks=[CustomMetrics(),  tensorboard])
    K.set_value(tau, np.max([K.get_value(tau) * np.exp(- 0.12), min_temperature]))
...
x_test_vae = ae.predict([aFrame, neibF, cut_neibF, weight_neibF, digs1hot])
len(x_test_vae)
...
data3D=np.loadtxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_encBatch10_VAE_noCut.txt')
...
#assign markers to latent variables
