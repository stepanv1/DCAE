...
ID='BCELLS'
'''
source_dir = "/mnt/f/Brinkman group/current/Stepan"
#file_list = glob.glob(source_dir + '/*.txt')
data0 = np.genfromtxt(source_dir + "/BreastCancerMeth/" +"Bcellsin5Transform.txt"
, names=None, dtype=float, skip_header=1)
aFrame = data0[:1000,:38] 

lbls=np.genfromtxt(source_dir + "/BreastCancerMeth/" + "labelsBcell.txt"
, names=None, dtype=int, skip_header=1)

lbls = [line.strip() for line in open(source_dir + "/BreastCancerMeth/" + "labelsBcell.txt", 'r')]
lbls=lbls[1:]
...
'''
outfile = 'BcellAsinh5TransformManhattan.npz'
...
cutgoff=npzfile['cutoff']; Dist =npzfile['Dist']
...
def Perturbation(i):
    sample=np.stack([np.random.choice(neibF[i,:,j],  1, p=weight_neibF[i,:]) for j in range(original_dim)],axis=-1)
    return sample
inputsS = np.random.choice(range(nrow),  szv)
resSample = Parallel(n_jobs=12, verbose=0, backend="threading")(delayed(Perturbation, check_pickle=False)(i) for i in inputsS)
pert = np.vstack(resSample)
del resSample
print('done...')
...
#validation set
idv= inputsS
targetVal = aFrame[idv,]
neibF_Val = neibF[idv,:,:]
cut_neibF_Val = cut_neibF[idv,:]
weight_neibF_Val = weight_neibF[idv,:]
sourceVal = pert#[idv, ]
