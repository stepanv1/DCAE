...
cutoff =np.ones(original_dim)*0.1
...
# Create AE and training
    ...
    ae.fit(X_train_tmp, X_train_tmp, batch_size=50, epochs=20)
...
x0 = Input(shape=(original_dim,))
h0 = Dense(intermediate_dim, activation='relu')(x0)

#h.set_weights(ae.layers[1].get_weights())
z_mean0 = Dense(latent_dim)(h0)
z_log_var0 = Dense(latent_dim)(h0)


def sampling(args):
    z_mean0, z_log_var0 = args
    epsilon = 0.0001*K.random_normal(shape=(K.shape(z_mean0)[0], latent_dim), mean=0.,
                              stddev=epsilon_std)
    return z_mean0 + K.exp(z_log_var0 / 2) * epsilon

# note that "output_shape" isn't necessary with the TensorFlow backend
z0 = Lambda(sampling, output_shape=(latent_dim,))([z_mean0, z_log_var0])

# we instantiate these layers separately so as to reuse them later
decoder_h0= Dense(intermediate_dim, activation='relu')
decoder_mean0 = Dense(original_dim, activation='relu')
h_decoded0 = decoder_h0(z0)
x_decoded_mean0 = decoder_mean0(h_decoded0)
ae = Model(x0, x_decoded_mean0)
#ae = Model([x, neib, cut_neib], x_decoded4)
ae.summary()
ae.compile(loss='mean_squared_error', optimizer='RMSprop')
ae.fit(aFrame, aFrame, batch_size=50, epochs=40)
...
#np.savetxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_encBatch10_VAE_noSigma.txt', x_test_enc)
