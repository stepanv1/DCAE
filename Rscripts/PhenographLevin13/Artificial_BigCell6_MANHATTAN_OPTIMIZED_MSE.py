...
model_name = 'Estimated NN_6_small_Manhattan'
...
ae_nn.compile(optimizer='adadelta', loss=tf.keras.losses.MeanAbsoluteError())

history_DAEnn= ae_nn.fit(X_train, mean_neib_train, epochs=2000, batch_size=1024,
                      shuffle=True,
                      validation_data=(X_test, mean_neib_test),
                verbose=2)
plt.plot(history_DAEnn.history['loss'][000:])
plt.plot(history_DAEnn.history['val_loss'][000:])
