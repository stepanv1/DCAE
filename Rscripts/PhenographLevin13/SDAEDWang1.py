...
def find_neighbors(data, k_, metric='L2', cores=12):
    ...
    return {'idx':res[0], 'dist': np.int32(res[1])}
...
data0 = np.genfromtxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/AllPatients_data0.txt', names=None, dtype=float, skip_header=1)
...
Idx, Dist =vp.find_nearest_neighbors(aFrame, k, 12)

'''
outfile = 'Mattmp0.npz'
#np.savez(outfile, Idx=Idx, aFrame=aFrame, lbls=lbls, cutoff=cutoff, patient=patient)
npzfile = np.load(outfile)
lbls=npzfile['lbls'];Idx=npzfile['Idx'];aFrame=npzfile['aFrame'];patient=npzfile['patient'];
cutoff=npzfile['cutoff'];

#from  sklearn.neighbors import BallTree
#tree = BallTree(aFrame, leaf_size=40,  metric="euclidean")
#dist, ind = tree.query(aFrame[:1000,], k=30)
...
len(x_test_vae)
np.savetxt('/mnt/f/Brinkman group/current/Stepan/WangData/WangDataPatient/x_test_ae001.txt',
           x_test_vae)
...
Y0all =  bh_sne(aFrame, max_iter=5000)

Y1all = bh_sne(x_test_vae, max_iter=5000)

#np.savez("tsne5000.npz", Y0all=Y0all, Y1all=Y1all)
