...
void Perplexity( double* dist,  int N, int D,   double* P, double perplexity,  int K, double* Sigma, int num_threads) {
    ...
    for(int n = 0; n < N; n++) {
        ...
        while(!found && iter < 200) {
            ...
            for(int m = 0; m < K; m++) cur_P[m] = exp(-beta * dist[row_P[n]+m] * dist[row_P[n]+m]);

            // Compute entropy of current row
            sum_P = DBL_MIN;
            for(int m = 0; m < K; m++) sum_P += cur_P[m];
            double H = .0;
            for(int m = 0; m < K; m++) H += beta * (dist[row_P[n]+m] * dist[row_P[n]+m] * cur_P[m]);
            H = (H / sum_P) + log(sum_P);
