...
def train_epoch(
    ...
    for (xb,) in loader:
        ...
        if use_dcae and lam_dcae > 0.0:
            pen = dcae_contractive_penalty_hutchinson_from_z(
    ...
    return {
        "loss": (mse_mean + lam_dcae * dcae_mean),
        "mse": mse_mean,
        "dcae": dcae_mean,
    }
...
def eval_epoch(
    ...
    for (xb,) in loader:
        ...
        if use_dcae and lam_dcae > 0.0:
            # Hutchinson estimator of ||∂z/∂x||_F^2, evaluated w.r.t. inputs only.
            n_probes = max(1, int(dcae_probes))
            B = xb.shape[0]
            pen = xb.new_tensor(0.0)

            for _ in range(n_probes):
                # Fresh input graph each probe → safe with retain_graph=False
...
def run_training(
    ...
    X_tr: torch.Tensor,
    X_val: torch.Tensor,
    cell_types=None,          # may come from **data, not used inside training
    *,
    m: Optional[int] = None,  # now OPTIONAL
    k: int = 2,
    batch_size: int = 4096,
    epochs_mse: int = 25,
    epochs_dcae: int = 25,
    ...
    Train the autoencoder in two phases:
      ...
      Phase-2: MSE + λ * DCAE (optional if lambda_dcae > 0 and epochs_dcae > 0).
    ...
    # ------------------------------------------------------------------
    # Phase 1: MSE-only  (UNCHANGED LOGIC FROM HERE DOWN)
    # ------------------------------------------------------------------
    ...
    if not use_dcae:
        print(
            "[INFO] lambda_dcae=0 or epochs_dcae=0 → "
            "DCAE is bypassed (MSE-only)."
        )
        ...
        print(
            "[Phase 2] switching to EAGER mode to support higher-order autograd."
        )
